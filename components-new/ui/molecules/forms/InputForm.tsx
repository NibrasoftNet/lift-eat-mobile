...
type InputFormType = 'text' | 'email' | 'password' | 'number' | 'pin' | 'search';
...
const InputForm = forwardRef<TextInput, InputFormProps>(
  (
    ...
    const [internalState, setInternalState] = React.useState<InputFormState>(state);
    ...
    const showStatusIcon = internalState === 'success' || internalState === 'error';
    // Utiliser les icônes importées 
const successIcon = <TickSquareRegularBoldIcon size={20} color={theme.color('success')} />
const errorIcon = <DangerRegularBoldIcon size={20} color={theme.color('error')} />
const statusIcon = internalState === 'success' ? successIcon : errorIcon;
    ...
    return (
      <Box 
        style={[
          { width: fullWidth ? undefined : 'auto', flex: fullWidth ? 1 : undefined } as ViewStyle, // Remplace width: '100%' par flex: 1
      ...
      >
        ...
        <View
          style={[
            styles.inputContainer,
            {
              ...
              borderWidth: (internalState === 'success' || internalState === 'error') ? 2 : 1, // 2px pour succès et erreur selon Figma
            }
          ]}
        >
          {/* Left Icon */}
          ...
          <TextInput
            ...
            style={[
              styles.input,
              {
                paddingHorizontal: leftIcon || rightIcon || showStatusIcon ? theme.space('xs') : 32, // 32px horizontalement selon Figma
        ...
        {/* Description */}
        {description && internalState !== 'error' && internalState !== 'success' && (
          <Text
            variant="caption"
