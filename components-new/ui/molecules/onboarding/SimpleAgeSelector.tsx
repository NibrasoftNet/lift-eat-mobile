...
/**
 * Version simplifiée du sélecteur d'âge pour l'onboarding
 */
export const SimpleAgeSelector: React.FC<SimpleAgeSelectorProps> = ({
  ...
  const renderSelector = (
    ...
    formatValue: (val: number) => string = (val) => String(val).padStart(2, '0')
  ) => {
    ...
    return (
      <View style={styles.selectorColumn}>
        <Text style={[styles.label, { color: textColors.label }]}>
          {title}
        </Text>
          ...
          {values.map((value, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.valueItem,
                  isSelected && { backgroundColor: dark ? '#2A2E37' : '#F0F0F0' }
                ]}
              ...
              >
                <Text 
                  style={[
                    styles.valueText,
                    {
                      color: isSelected ? textColors.highlight : textColors.primary,
  ...
  return (
    <Box style={[styles.container, { backgroundColor }]}>
      <View style={styles.selectors}>
        {renderSelector('Month', months, selectedMonth, setSelectedMonth, formatMonth)}
        {renderSelector('Day', days, selectedDay, setSelectedDay, formatDay)}
        {renderSelector('Year', years, selectedYear, setSelectedYear, formatYear)}
      </View>
      
      <View style={styles.selectedDate}>
        <Text style={[styles.dateText, { color: textColors.highlight }]}>
          {`${formatMonth(selectedMonth)}/${formatDay(selectedDay)}/${formatYear(selectedYear)}`}
        </Text>
      </View>
    </Box>
  );
...
export default SimpleAgeSelector;
