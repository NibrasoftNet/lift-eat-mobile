...
export type DietaryRestrictionFormType = z.infer<typeof dietaryRestrictionSchema>;

// Schéma pour un jour du plan
export const planDaySchema = z.object({
  dayNumber: z.number().min(1).max(30),
  includedMealTypes: z.array(z.nativeEnum(MealTypeEnum)).min(1, "Au moins un type de repas doit être sélectionné"),
...
const baseFormSchema = z.object({
  ...
  numberOfDays: z.number().min(1, "Le nombre de jours doit être au moins 1").max(30, "Le nombre de jours maximum est 30"),
  totalCalories: z.number().min(500, "Les calories totales doivent être au moins 500").optional(),
  caloriesPerDay: z.number().min(500, "Les calories par jour doivent être au moins 500").optional(),
  targetWeight: z.number().positive("Le poids cible doit être positif").optional(),
...
export const planGeneratorFormSchema = baseFormSchema.refine(
  (data) => {
    ...
    if ((data.goal === GoalEnum.WEIGHT_LOSS || data.goal === GoalEnum.GAIN_MUSCLE) && !data.targetWeight) {
      return false;
    }
    return true;
  },
  {
    message: "Le poids cible est obligatoire pour les objectifs de perte de poids ou prise de muscle",
...
export const defaultPlanGeneratorFormValues: PlanGeneratorFormType = {
  ...
  numberOfDays: 7,
  includedMealTypes: [MealTypeEnum.BREAKFAST, MealTypeEnum.LUNCH, MealTypeEnum.DINNER],
  dietaryRestrictions: [],
