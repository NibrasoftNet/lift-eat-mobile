...
export function normalizeCuisineType(cuisine: string): CuisineTypeEnum {
  ...
  logger.debug(
    LogCategory.IA,
    `normalizeCuisineType: Cuisine non reconnue "${cuisine}", utilisation de GENERAL par défaut`,
  );
...
export function normalizeMealUnit(unit: string): MealUnitEnum {
  ...
  logger.debug(
    LogCategory.IA,
    `normalizeMealUnit: Unité non reconnue "${unit}", utilisation de GRAMMES par défaut`,
  );
...
export const validateMeal = (data: string) => {
  try {
    ...
    logger.debug(
      LogCategory.IA,
      `validateMeal: données brutes reçues: ${data.substring(0, 200)}...`,
    );
    ...
    try {
      ...
      logger.debug(
        LogCategory.IA,
        `validateMeal: erreur de parsing initial: ${
      ...
      logger.debug(
        LogCategory.IA,
        `validateMeal: JSON nettoyé: ${cleanedData.substring(0, 200)}...`,
      );
      ...
      try {
        ...
        logger.error(
          LogCategory.IA,
          `validateMeal: échec de parsing même après nettoyage: ${
    ...
    if (!jsonData.name) {
      logger.debug(
        LogCategory.IA,
        `validateMeal: Validation échouée - nom du repas manquant`,
      );
      return {
        success: false,
    ...
    logger.debug(
      LogCategory.IA,
      `validateMeal: Propriétés disponibles dans le JSON: ${Object.keys(
        jsonData,
      ).join(', ')}`,
    );
...
export const validatePlan = (data: string) => {
  try {
    ...
    try {
      ...
      logger.debug(
        LogCategory.IA,
        `validatePlan: Parsing JSON réussi après nettoyage`,
      );
    } catch (parseError) {
      // 3. Si le parsing a échoué, essayer des méthodes de récupération alternatives
      logger.warn(
        LogCategory.IA,
        `validatePlan: Première tentative de parsing échouée: ${
          parseError instanceof Error ? parseError.message : String(parseError)
        }`,
      );
      ...
      try {
        ...
        const potentialJson = cleanedData
          .replace(/^[^{]*/, '')
          .replace(/[^}]*$/, '');
        if (potentialJson.includes('{') && potentialJson.includes('}')) {
          ...
          logger.debug(
            LogCategory.IA,
            `validatePlan: Récupération JSON réussie avec extraction partielle`,
          );
        } else {
        ...
        logger.error(
          LogCategory.IA,
          `validatePlan: Échec de récupération JSON: ${
        ...
        if (nameMatch && goalMatch) {
          ...
          logger.debug(
            LogCategory.IA,
            `validatePlan: Récupération heuristique des données réussie`,
            { extractedData: jsonData },
          );
        } else {
          throw new Error(
            "Impossible d'extraire les données minimales du plan",
          );
        }
      }
    }
    ...
    if (jsonData.goal) {
      // Convertir en majuscules sans accents et espaces pour la comparaison
      const normalizedGoal = jsonData.goal
        .toString()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toUpperCase()
      ...
      if (!Object.values(GoalEnum).includes(jsonData.goal as GoalEnum)) {
        logger.warn(
          LogCategory.IA,
          `validatePlan: Objectif non reconnu "${jsonData.goal}", utilisation de l'objectif demandé par l'utilisateur`,
        );
    ...
    if (
      ...
      logger.warn(
        LogCategory.IA,
        `validatePlan: Aucun repas trouvé, création d'un repas par défaut`,
      );
    ...
    logger.debug(
      LogCategory.IA,
      `validatePlan: Données préparées pour validation`,
      {
        dataPreview:
    ...
    if (result.success) {
      logger.info(
        LogCategory.IA,
        `validatePlan: Validation réussie pour le plan "${result.data.name}"`,
      );
      return {
        success: true,
        data: result.data,
      ...
      logger.warn(
        LogCategory.IA,
        `validatePlan: Validation échouée: ${errorMessages}`,
      );
      return {
        success: false,
        errors: result.error,
