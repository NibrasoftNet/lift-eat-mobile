...
import {
  FormOperationResult,
  NutritionGoalFormServiceInterface,
...
class NutritionGoalFormService implements NutritionGoalFormServiceInterface {
  ...
  validateUserAccess(
    ...
    if (!userId) {
      logger.warn(
        LogCategory.AUTH,
        'User not authenticated when accessing nutrition goal form',
      );
      toast.show({
        placement: 'top',
        render: ({ id }: { id: string }) => {
    ...
    if (userId !== targetUserId) {
      logger.warn(
        LogCategory.AUTH,
        `User ID mismatch: authenticated=${userId}, target=${targetUserId}`,
      );
      toast.show({
        placement: 'top',
        render: ({ id }: { id: string }) => {
  ...
  async submitForm(
    ...
    if (!userId) {
      logger.error(
        LogCategory.AUTH,
        'User not authenticated when creating nutrition plan',
      );
      return {
        success: false,
    ...
    try {
      ...
      logger.debug(
        ...
        'Nutrition goal form data prepared for submission',
        planData,
      );
      ...
      const errorMessage = `Error preparing nutrition goal data: ${
  ...
  prepareDefaultValues(
    ...
    if (!defaultValues) {
      logger.warn(
        LogCategory.FORM,
        'No default values provided for nutrition goal form',
      );
      return {
  ...
  handleGoalUnitChange(goalUnit: string, setValue: Function): void {
    ...
    if (goalUnit === GoalEnum.MAINTAIN) {
      logger.debug(
        LogCategory.FORM,
        'Goal is MAINTAIN, synchronizing target weight with initial weight',
      );
