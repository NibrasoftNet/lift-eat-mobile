  ...
  async submitForm(data: UserGenderActivityFormData, userId: string, operation: 'create' | 'update'): Promise<FormOperationResult> {
    try {
      ...
      const userPreferences: LocalUserPreferences = {
        ...
        dietaryRestrictions: data.dietaryRestrictions as DietaryRestrictionEnum[],
      ...
      const resultPreferences = await sqliteMCPServer.updateUserPreferencesViaMCP(userIdNumber, preferencesForUpdate);
      
      if (!resultPreferences || !resultPreferences.success) {
        throw new Error(resultPreferences?.error || 'Failed to update basic user preferences');
      }
      
      // Mettre à jour les préférences nutritionnelles dans une table séparée
      logger.debug(LogCategory.FORM, 'Updating nutritional preferences with dietaryRestrictions:', userPreferences.dietaryRestrictions);
      logger.debug(LogCategory.FORM, 'Updating nutritional preferences with allergies:', userPreferences.allergies);
      logger.debug(LogCategory.FORM, 'Updating nutritional preferences with nutritionGoals:', userPreferences.nutritionGoals);
      ...
      if (userPreferences.nutritionGoals) {
        ...
        if (userPreferences.physicalActivity === PhysicalActivityEnum.SEDENTARY) {
          dailyCalories = dailyCalories * 0.8; // 20% de moins pour les sédentaires
        } else if (userPreferences.physicalActivity === PhysicalActivityEnum.HIGH) {
        ...
        logger.debug(LogCategory.NUTRITION, 'Valeurs nutritionnelles calculées automatiquement:', nutritionGoalsForUpdate);
      }
      
      // Mettre à jour les préférences nutritionnelles avec les valeurs calculées
      const resultNutrition = await sqliteMCPServer.updateUserNutritionPreferencesViaMCP(
        userIdNumber,
      ...
      if (!resultNutrition || !resultNutrition.success) {
        throw new Error(resultNutrition?.error || 'Failed to update nutritional preferences');
      }
