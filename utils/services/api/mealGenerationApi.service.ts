...
export const mealGenerationApiService = {
   ...
   */
  async createMeal(meal: IaMealType, userId: number): Promise<CreateMealResponse> {
    try {
      logger.debug(LogCategory.IA, `Création du repas ${meal.name} pour l'utilisateur ${userId}`, 'createMeal');
      
      // Utiliser le MCP Server pour ajouter le repas
      const result = await sqliteMCPServer.addMealViaMCP(meal, userId);
      
      if (!result.success) {
        logger.error(LogCategory.IA, `Erreur lors de la création du repas: ${result.error}`, 'createMeal');
        return {
          success: false,
          error: result.error
        };
      ...
      if (queryClient) {
        logger.debug(LogCategory.IA, `Invalidation du cache des repas pour l'utilisateur ${userId}`, 'createMeal');
      ...
      logger.info(LogCategory.IA, `Repas ${meal.name} créé avec succès (ID: ${result.mealId})`, 'createMeal');
      ...
      logger.error(LogCategory.IA, `Erreur lors de la création du repas: ${error instanceof Error ? error.message : String(error)}`, 'createMeal');
  ...
  async getMealById(mealId: number, userId: number): Promise<any> {
    try {
      logger.debug(LogCategory.IA, `Récupération du repas ${mealId} pour l'utilisateur ${userId}`, 'getMealById');
      ...
      logger.error(LogCategory.IA, `Erreur lors de la récupération du repas: ${error instanceof Error ? error.message : String(error)}`, 'getMealById');
      
      throw new IaError(
        `Erreur lors de la récupération du repas: ${error instanceof Error ? error.message : String(error)}`,
  ...
  async searchMeals(params: SearchMealParams): Promise<any[]> {
    try {
      logger.debug(LogCategory.IA, `Recherche de repas pour l'utilisateur ${params.userId}`, 'searchMeals');
      ...
      logger.error(LogCategory.IA, `Erreur lors de la recherche de repas: ${error instanceof Error ? error.message : String(error)}`, 'searchMeals');
      
      throw new IaError(
        `Erreur lors de la recherche de repas: ${error instanceof Error ? error.message : String(error)}`,
        IaErrorType.API_ERROR,
        error
      );
    }
  ...
  async deleteMeal(mealId: number, userId: number): Promise<boolean> {
    try {
      logger.debug(LogCategory.IA, `Suppression du repas ${mealId} pour l'utilisateur ${userId}`, 'deleteMeal');
      
      // Vérifier que le repas appartient à l'utilisateur
      const mealResult = await mealService.getMealDetails(mealId);
      if (mealResult.success && mealResult.meal && mealResult.meal.creatorId !== userId) {
        throw new IaError(
          `Repas ${mealId} n'appartient pas à l'utilisateur ${userId}`,
          IaErrorType.UNAUTHORIZED_ERROR
        );
      ...
      const queryClient = getQueryClient();
      if (queryClient) {
        logger.debug(LogCategory.IA, `Invalidation du cache des repas pour l'utilisateur ${userId}`, 'deleteMeal');
        await queryClient.invalidateQueries({
          predicate: (query) => {
            const queryKey = query.queryKey[0];
            return typeof queryKey === 'string' && queryKey.startsWith('my-meals');
      ...
      logger.info(LogCategory.IA, `Repas ${mealId} supprimé avec succès`, 'deleteMeal');
      
      return true;
    } catch (error) {
      logger.error(LogCategory.IA, `Erreur lors de la suppression du repas: ${error instanceof Error ? error.message : String(error)}`, 'deleteMeal');
      
      throw new IaError(
        `Erreur lors de la suppression du repas: ${error instanceof Error ? error.message : String(error)}`,
        IaErrorType.API_ERROR,
        error
      );
    }
