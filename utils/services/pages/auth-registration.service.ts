...
class AuthRegistrationService {
  ...
  async finalizeRegistration(
    ...
    try {
      logger.info(
        LogCategory.AUTH,
        "Finalisation de l'inscription après vérification OTP",
        {
          clerkId: clerkUserId,
          email,
        },
      ...
      logger.info(
        LogCategory.USER,
        'Création utilisateur SQLite après vérification OTP',
        {
          clerkId: clerkUserId,
          email: email,
        },
      ...
      if (!sqliteResult.success) {
        logger.error(
          LogCategory.USER,
          'Échec création utilisateur SQLite après vérification OTP',
          {
        ...
        return {
          ...
          error:
            sqliteResult.error ||
            "Échec de création de l'utilisateur dans SQLite",
        };
      }
      ...
      if (
        !sqliteResult.data ||
        typeof sqliteResult.data.userId === 'undefined'
      ) {
        logger.error(
          LogCategory.USER,
          'ID SQLite manquant dans le résultat après création',
          {
      ...
      logger.info(
        LogCategory.USER,
        'Utilisateur créé avec succès dans SQLite après vérification OTP',
        {
          email,
          name: normalizedUserName,
          clerkId: clerkUserId,
      ...
      logger.error(
        LogCategory.AUTH,
        "Exception lors de la finalisation de l'inscription",
        {
