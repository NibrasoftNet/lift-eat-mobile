...
class UserPagesService implements UserPagesServiceInterface {
  ...
  async createUser(userData: {
    ...
    try {
      ...
      if (userData.id) {
        const existsResult = await userCoreService.validateUserExists(
          userData.id,
        );
        if (existsResult.success && existsResult.data?.exists) {
          logger.info(
            LogCategory.USER,
            "L'utilisateur avec cet ID existe déjà, mise à jour",
            { id: userData.id },
          );
      ...
      logger.info(
        LogCategory.USER,
        "Création d'un nouvel utilisateur dans SQLite",
        {
          id: userData.id,
          email: userData.email,
        },
      ...
      if (userData.id) {
        logger.info(
          LogCategory.USER,
          "Utilisation de l'ID spécifié pour la création",
          { id: userData.id },
        );
        ...
        userId =
          createResult.success && createResult.data?.userId
        ...
        if (createResult.success) {
          if (createResult.data?.userId) {
            ...
            logger.info(
              LogCategory.USER,
              'ID utilisateur récupéré depuis data.userId',
              { userId },
            );
            ...
            logger.info(
              LogCategory.USER,
              'ID utilisateur récupéré depuis userId',
              { userId },
            );
          } else {
            userId = null;
            logger.warn(
              LogCategory.USER,
              'Aucun ID utilisateur trouvé dans le résultat',
              { createResult },
            );
          }
        } else {
      ...
      if (!userId) {
        logger.error(
          LogCategory.USER,
          'ID utilisateur non disponible après création',
          {
            result: createResult,
          },
        );
      ...
      if (userData.clerkId) {
        logger.info(
          LogCategory.USER,
          "Association de l'ID Clerk à l'utilisateur SQLite",
          {
            userId,
        ...
        try {
          ...
          if (!userDetailsResult.success || !userDetailsResult.user) {
            throw new Error(
              `Impossible de récupérer les détails de l'utilisateur avec ID ${userId}`,
            );
          }

          // Utiliser userCoreService pour mettre à jour l'utilisateur avec toutes les propriétés requises
          const updateResult = await userCoreService.updateUserProfileData(
            userId,
            {
          ...
          if (updateResult.success) {
            ...
            logger.warn(
              LogCategory.USER,
              'Avertissement: ID Clerk non associé mais utilisateur créé',
              {
          ...
          logger.error(
            LogCategory.USER,
            "Erreur lors de l'association de l'ID Clerk",
            {
      ...
      if (userData.convexId) {
        logger.info(
          LogCategory.USER,
          "ID Convex associé à l'utilisateur (non enregistré)",
          {
            userId,
      ...
      const preferencesResult = await userCoreService.updateUserPreferences(
        userId,
        {
          age: 30,
          gender: GenderEnum.MALE,
      ...
      if (!preferencesResult || !preferencesResult.success) {
        logger.warn(
          LogCategory.USER,
          'Erreur lors de la création des préférences utilisateur',
          {
      ...
      logger.info(
        LogCategory.USER,
        'Utilisateur créé avec succès dans SQLite',
        { id: userId },
      );
      return {
        success: true,
      ...
      logger.error(
        LogCategory.USER,
        "Exception lors de la création de l'utilisateur",
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  async getUserProfile(
    id?: number,
  ): Promise<OperationResult<{ user: any; details: any; preferences: any }>> {
