...
class NutritionPagesService {
  ...
  async calculateCaloriesIntake(userProfile: {
    ...
    try {
      const dailyCalories =
        nutritionCoreService.calculateCaloriesIntake(userProfile);
   ...
   */
  async updateUserNutritionPreferences(
    userId: number,
    preferences: any,
  ): Promise<OperationResult<any>> {
    try {
      // Déléguer à userService via sqliteMCPServer
      const result = await nutritionCoreService.updateUserNutritionPreferences(
        userId,
        preferences,
      );
  ...
  async createPlan(
    ...
    try {
      logger.info(
        LogCategory.DATABASE,
        "Demande de création d'un plan nutritionnel",
        { userId },
      );
      ...
      logger.error(
        LogCategory.DATABASE,
        'Erreur lors de la demande de création de plan',
        {
          userId,
          error: error instanceof Error ? error.message : String(error),
        },
   ...
   */
  async getMealNutritionForDisplay(
    mealId: number,
    quantity?: number,
  ...
  > {
    try {
      logger.info(
        ...
        `[MEALCARD-NUTRITION] Chargement des données nutritionnelles pour le repas ${mealId}`,
      );

      // Utiliser la quantité fournie ou obtenir le poids standard
      const finalQuantity =
        quantity || (await this.getDefaultMealWeight(mealId));

      // Déléguer directement à nutritionEngine (qui loggera ses propres actions)
      const result = await nutritionEngine.getMealNutrition(
        mealId,
        finalQuantity,
        displayMode,
      );
      ...
      logger.debug(
        LogCategory.NUTRITION,
        `[MEALCARD-NUTRITION] Données reçues avec succès pour le repas ${mealId}`,
      );
      ...
      logger.error(
        LogCategory.NUTRITION,
        `[MEALCARD-NUTRITION] Erreur pour le repas ${mealId}`,
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  private async getDefaultMealWeight(mealId: number): Promise<number> {
    try {
      ...
      logger.info(
        LogCategory.NUTRITION,
        `[MEALCARD-CALORIES] Poids: ${weight}g`,
      );
      return weight;
    } catch (error) {
      logger.warn(
        LogCategory.NUTRITION,
        `Erreur lors de la récupération du poids, utilisation de 100g`,
        {
          mealId,
        },
      );
   ...
   */
  async getDailyPlanMacrosForDisplay(
    dailyPlanId: number,
  ...
  > {
    try {
      logger.info(
        LogCategory.NUTRITION,
        `Préparation des données nutritionnelles pour affichage: plan ${dailyPlanId}`,
      );

      // Utiliser la façade pour récupérer les données
      const result = await nutritionEngine.getPlanNutrition(
        dailyPlanId,
        displayMode,
      );
      ...
      if (!result.success) {
        return {
          ...
          error:
            result.error ||
            'Erreur lors de la récupération des données nutritionnelles du plan',
          data: {
            macros: {
              calories: 0,
              carbs: 0,
      ...
      logger.error(
        LogCategory.NUTRITION,
        `Erreur lors de la préparation des données nutritionnelles du plan pour affichage`,
        {
          error: error instanceof Error ? error.message : String(error),
          dailyPlanId,
          displayMode,
   ...
   */
  async getMacroBreakdownForDisplay(
    mealId: number,
    quantity?: number,
    displayMode: NutritionDisplayMode = NutritionDisplayMode.PER_100G,
  ): Promise<
  ...
  > {
    try {
      logger.info(
        LogCategory.NUTRITION,
        `Préparation de la répartition des macros pour affichage: repas ${mealId}`,
      );

      // Utiliser la façade pour récupérer les données
      const result = await nutritionEngine.getMacroBreakdown(
        mealId,
        quantity,
        displayMode,
      );
      ...
      if (!result.success) {
        return {
          ...
          error:
            result.error ||
            'Erreur lors de la récupération de la répartition des macros',
          data: {
            macros: {
              calories: 0,
              carbs: 0,
      ...
      logger.error(
        LogCategory.NUTRITION,
        `Erreur lors de la préparation de la répartition des macros pour affichage`,
        {
          error: error instanceof Error ? error.message : String(error),
          mealId,
          quantity,
