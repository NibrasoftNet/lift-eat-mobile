...
export interface OnboardingPagesServiceInterface {
   ...
   */
  saveUserData(data: Partial<UserOnboardingData>): Promise<OperationResult<void>>;
...
class OnboardingPagesService implements OnboardingPagesServiceInterface {
   ...
   * Sauvegarde les données d'onboarding
   */
  async saveUserData(data: Partial<UserOnboardingData>): Promise<OperationResult<void>> {
    try {
      logger.info(LogCategory.USER, "Sauvegarde des données d'onboarding", { data });
      await OnboardingService.saveUserData(data);
      return {
        success: true
      };
    } catch (error) {
      logger.error(LogCategory.USER, "Erreur lors de la sauvegarde des données d'onboarding", {
        error: error instanceof Error ? error.message : String(error),
        data
      });
      return {
        success: false,
        error: error instanceof Error ? error.message : "Une erreur est survenue lors de la sauvegarde des données d'onboarding"
      };
    }
  }
   ...
   * Récupère les données d'onboarding
   */
  async getUserData(): Promise<OperationResult<Partial<UserOnboardingData> | null>> {
    try {
      ...
      logger.error(LogCategory.USER, "Erreur lors de la récupération des données d'onboarding", {
        error: error instanceof Error ? error.message : String(error)
      });
      return {
        success: false,
        error: error instanceof Error ? error.message : "Une erreur est survenue lors de la récupération des données d'onboarding"
      };
    }
  }
  ...
  async isOnboardingCompleted(): Promise<OperationResult<boolean>> {
    try {
      ...
      logger.error(LogCategory.USER, "Erreur lors de la vérification du statut d'onboarding", {
        error: error instanceof Error ? error.message : String(error)
      });
      return {
        success: false,
        error: error instanceof Error ? error.message : "Une erreur est survenue lors de la vérification du statut d'onboarding"
      };
    }
  }
  ...
  async completeOnboarding(userId?: number): Promise<OperationResult<void>> {
    try {
      ...
      if (!userData) {
        logger.error(LogCategory.USER, "Aucune donnée d'onboarding trouvée lors de la complétion");
        return {
          success: false,
      ...
      logger.info(LogCategory.USER, "Vérification des données d'onboarding avant synchronisation", { 
      ...
      if (userId && userData) {
        ...
        if (isNaN(numericUserId)) {
          logger.error(LogCategory.USER, "ID utilisateur invalide lors de la synchronisation", { userId });
          return {
            success: false,
        ...
        console.log('[DEBUG] Synchronisation des données d\'onboarding avec la base de données pour utilisateur ID:', numericUserId);
        logger.info(LogCategory.USER, 'Synchronisation des données d\'onboarding', { userId: numericUserId });
        ...
        if (userData.birthDate) {
          ...
          console.log('[DEBUG] Âge calculé à partir de la date de naissance:', age);
        }
        ...
        console.log('[DEBUG] Données à synchroniser avec la BDD:', JSON.stringify(userPreferences, null, 2));
        
        // Synchroniser avec la base de données via userPagesService
        const dbResult = await userPagesService.updateUserPreferences(numericUserId, userPreferences);
        
        if (!dbResult.success) {
          logger.warn(LogCategory.USER, 'Synchronisation partielle: données enregistrées localement mais pas en BDD', { 
            error: dbResult.error,
            userId: numericUserId
          });
          console.warn('[DEBUG] Données enregistrées localement mais pas synchronisées avec la base de données:', dbResult.error);
          
          // Retourner une réussite partielle car les données sont au moins sauvegardées localement
          return {
            success: true,
            warning: "Les données ont été sauvegardées localement mais n'ont pas pu être synchronisées avec la base de données"
          };
        } else {
          console.log('[DEBUG] Données d\'onboarding synchronisées avec la base de données avec succès!');
          logger.info(LogCategory.USER, 'Données d\'onboarding synchronisées avec succès', { userId: numericUserId });
        }
      } else {
        logger.warn(LogCategory.USER, 'Impossible de synchroniser les données d\'onboarding: userId manquant', { 
          hasUserData: !!userData 
        });
        console.warn('[DEBUG] Impossible de synchroniser les données d\'onboarding: userId manquant');
        
        // Retourner une réussite partielle car les données sont au moins sauvegardées localement
        return {
          success: true,
          warning: "Les données ont été sauvegardées localement mais n'ont pas pu être synchronisées avec la base de données (ID utilisateur manquant)"
        };
      }
      ...
      logger.error(LogCategory.USER, "Erreur lors de la complétion de l'onboarding", {
        error: error instanceof Error ? error.message : String(error)
      });
      return {
        success: false,
        error: error instanceof Error ? error.message : "Une erreur est survenue lors de la complétion de l'onboarding"
      };
    }
  }
  ...
  calculateBMI(
    ...
    try {
      const bmi = OnboardingService.calculateBMI(height, weight, heightUnit, weightUnit);
      return {
        success: true,
        data: bmi
      };
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Une erreur est survenue lors du calcul de l'IMC"
      };
    }
  }
}
