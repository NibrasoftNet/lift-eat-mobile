...
class ProgressPagesService implements ProgressPagesServiceInterface {
  ...
  async getProgressHistory(startDate: string, endDate: string): Promise<OperationResult<{
    ...
    try {
      logger.info(LogCategory.APP, 'Récupération de l\'historique de progression demandé (service pages)', {
        startDate,
        endDate
      });
      
      // Délégation au service core
      const result = await progressService.getProgressHistory(startDate, endDate);
      ...
      return {
        ...
        message: `Historique de progression récupéré avec succès (${result.progressData?.length || 0} jours)`
      };
    } catch (error) {
      logger.error(LogCategory.APP, 'Erreur lors de la récupération de l\'historique de progression (service pages)', {
        error: error instanceof Error ? error.message : String(error),
        startDate,
        endDate
      });
  ...
  async updateProgress(progressId: number, progressData: any): Promise<OperationResult> {
    try {
      logger.info(LogCategory.APP, 'Mise à jour de la progression quotidienne (service pages)', { 
        progressId
      });
      
      // Vérifier que progressId est défini et valide
      if (!progressId || progressId <= 0) {
        logger.error(LogCategory.APP, 'ID de progression invalide', { progressId });
        return {
          success: false,
      ...
      const result = await progressService.updateDailyProgress(progressId, progressData);
      ...
      logger.error(LogCategory.APP, 'Erreur lors de la mise à jour de la progression quotidienne (service pages)', {
        error: error instanceof Error ? error.message : String(error),
        progressId
      });
  ...
  async markMealAsConsumed(
    ...
    try {
      logger.info(LogCategory.APP, 'Demande de marquage de repas via service de pages', {
        dailyProgressId,
        mealId,
        dailyPlanMealId,
        consumed
      ...
      logger.error(LogCategory.APP, 'Erreur lors du marquage du repas via service de pages', {
        error: error instanceof Error ? error.message : String(error),
        dailyProgressId,
        mealId
      });
