...
class ScannerPagesService {
  async scanBarcode(barcode: string, userId: number): Promise<ScanResult> {
    ...
    logger.info(LogCategory.INTEGRATION, '[SCANNER] scanBarcode (pages) params', { barcode, userId });
    const result = await scannerCoreService.scanBarcode(barcode);
    logger.info(LogCategory.INTEGRATION, '[SCANNER] scanBarcode (pages) core result', {
      barcode,
      userId,
    ...
    if (result.isValid && result.productResult) {
      logger.info(LogCategory.INTEGRATION, '[SCANNER] scanBarcode (pages) calling addHistory', {
        barcode,
        name: result.productResult.name || '',
        userId,
      });
      await scanHistoryCoreService.addHistory({ barcode, name: result.productResult.name || '', userId });
    }
    return result;
  }

  async searchProducts(params: SearchParams, userId: number): Promise<OperationResult<{ results: any[] }>> {
    try {
      const results = await scannerCoreService.searchProducts(params);
      return { success: true, data: { results } };
    } catch (error) {
      logger.error(LogCategory.INTEGRATION, '[SCANNER] searchProducts presenter error', {
  ...
  async getHistory(userId: number, limit = 20): Promise<OperationResult<GetScanHistoryResult>> {
    try {
      const results = await scanHistoryCoreService.getHistory(userId, limit);
      return { success: true, data: results };
    } catch (error) {
      logger.error(LogCategory.INTEGRATION, '[SCANNER] getHistory presenter error', {
        error: error instanceof Error ? error.message : String(error),
        userId,
        limit,
      });
   ...
   */
  async getProductDetails(barcode: string): Promise<{ success: boolean; productResult?: ProductResult; error?: string }> {
    try {
      const result = await scannerCoreService.scanBarcode(barcode);
      if (result.isValid && result.productResult) {
      ...
      logger.error(LogCategory.INTEGRATION, '[SCANNER] getProductDetails presenter error', {
        error: error instanceof Error ? error.message : String(error),
        barcode,
      });
  ...
  async clearScanHistory(userId: number): Promise<OperationResult> {
    try {
      ...
      logger.error(LogCategory.INTEGRATION, '[SCANNER] clearScanHistory presenter error', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
