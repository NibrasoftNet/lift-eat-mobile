...
class PlanPagesService implements PlanPagesServiceInterface {
   ...
   */
  async getPlansList(filters: PlanFilters): Promise<
    OperationResult<{
      plans: any[];
      totalCount: number;
      pageInfo?: {
   ...
   */
  async getPlanDetails(id: number): Promise<
    OperationResult<{
      plan: any;
      dailyPlans: any[];
    }>
    ...
    try {
      ...
      if (!result.success) {
        return {
          ...
          error:
            result.error ||
            `Échec de la récupération des détails du plan ${id}`,
        };
      }
      ...
      logger.error(
        LogCategory.UI,
        `Erreur lors de la récupération des détails du plan ${id}`,
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Une erreur est survenue lors de la récupération des détails du plan',
      };
    }
  }
  ...
  async deletePlan(id: number): Promise<OperationResult> {
    try {
      ...
      logger.error(
        LogCategory.UI,
        `Erreur lors de la suppression du plan ${id}`,
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  async updatePlan(id: number, planData: any): Promise<OperationResult> {
    try {
      ...
      logger.error(
        LogCategory.UI,
        `Erreur lors de la mise à jour du plan ${id}`,
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Une erreur est survenue lors de la récupération du plan courant',
      };
    }
  }
  ...
  async addMealToDailyPlan(
    ...
    try {
      ...
      // Déléguer l'opération au service core
      const result = await planService.addMealToDailyPlan(
        dailyPlanId,
        mealId,
        quantity,
        mealType,
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Une erreur est survenue lors de l'ajout du repas au plan journalier",
      };
    }
  }
  ...
  > {
    try {
      logger.info(
        LogCategory.UI,
        `Préparation des données pour la carte de repas: plan ${dailyPlanId}, repas ${mealId}`,
      );
      ...
      if (!userId) {
        return {
          success: false,
          error:
            'Utilisateur non authentifié pour accéder aux détails du repas',
        };
      }
      ...
      const mealResult = await sqliteMCPServer.getMealByIdWithIngredientsViaMCP(
        mealId,
        userId,
      );
      ...
      const quantityResult = await planService.getMealQuantityInPlan(
        dailyPlanId,
        mealId,
      );
      if (!quantityResult.success || quantityResult.quantity === undefined) {
        return {
          ...
          error:
            ...
            'Erreur lors de la récupération de la quantité du repas dans le plan',
        };
      }
      ...
      return {
        ...
        data: {
          meal: {
            ...
            imageUrl: mealResult.meal?.image
              ? `data:image/jpeg;base64,${mealResult.meal.image.toString(
                  'base64',
                )}`
              : '',
          ...
          displayText:
            nutritionResult.data.displayText || 'Valeurs nutritionnelles',
      ...
      logger.error(
        LogCategory.UI,
        `Erreur lors de la préparation des données pour la carte de repas`,
        {
          error: error instanceof Error ? error.message : String(error),
          dailyPlanId,
          mealId,
   ...
   */
  async getDailyPlanSummary(dailyPlanId: number): Promise<
    OperationResult<{
      planDetails: {
        id: number;
        name: string;
  ...
  > {
    try {
      logger.info(
        LogCategory.UI,
        `Préparation du résumé du plan journalier: ${dailyPlanId}`,
      );
      ...
      if (!planResult?.success) {
        return {
          ...
          error:
            planResult?.error ||
            'Erreur lors de la récupération des détails du plan journalier',
        };
      }

      // 2. Utiliser nutritionPagesService pour calculer les valeurs nutritionnelles totales du plan
      const nutritionResult =
        await nutritionPagesService.getDailyPlanMacrosForDisplay(
          dailyPlanId,
      ...
      if (!nutritionResult.success || !nutritionResult.data) {
        return {
          ...
          error:
            nutritionResult.error ||
            'Erreur lors du calcul des valeurs nutritionnelles du plan',
        };
      }
      ...
      return {
        ...
        data: {
          ...
          macros: nutritionResult.data.macros || defaultMacros,
          displayText:
            nutritionResult.data.displayText ||
            'Valeurs nutritionnelles du plan',
      ...
      logger.error(
        LogCategory.UI,
        `Erreur lors de la préparation du résumé du plan journalier`,
        {
          error: error instanceof Error ? error.message : String(error),
          dailyPlanId,
        },
    ...
    try {
      ...
      if (!result.success) {
        return {
          success: false,
          error:
            result.error || 'Erreur lors de la suppression du repas du plan',
        };
      }
  ...
  async setCurrentPlan(planId: number): Promise<OperationResult> {
    try {
      logger.info(
        LogCategory.UI,
        `Définition du plan ${planId} comme plan actuel`,
      );
      ...
      if (!result.success) {
        return {
          ...
          error:
            result.error ||
            'Erreur lors de la définition du plan comme plan actuel',
        };
      }
  ...
  invalidatePlanCache(
    ...
    try {
      logger.info(
        LogCategory.UI,
        `Invalidation du cache pour le plan ${planId}`,
      );
  ...
  async updateMealQuantityInPlan(
    ...
    try {
      logger.info(
        LogCategory.UI,
        `Mise à jour de la quantité du repas ${mealId} dans le plan ${dailyPlanId} à ${newQuantity}`,
      );
      ...
      if (!result.success) {
        return {
          ...
          error:
            result.error ||
            'Erreur lors de la mise à jour de la quantité du repas',
        };
      }
   ...
   */
  async getDailyPlanNutrition(dailyPlanId: number): Promise<
    OperationResult<{
      macros: MacroNutrientsBase;
    }>
  > {
    try {
      ...
      if (!result?.success || !result.nutrition) {
        return {
          ...
          error:
            result?.error ||
            'Erreur lors de la récupération des valeurs nutritionnelles',
        };
      }
      ...
      logger.error(
        ...
        `Erreur lors de la récupération des valeurs nutritionnelles du plan ${dailyPlanId}`,
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  async getPlanNutritionGoals(planId: number): Promise<
    OperationResult<{
      macros: MacroNutrientsBase;
      displayText: string;
    }>
  > {
    try {
      logger.info(
        LogCategory.NUTRITION,
        `Récupération des objectifs nutritionnels du plan ${planId}`,
      );
      ...
      if (!userId) {
        logger.error(
          LogCategory.NUTRITION,
          'Utilisateur non authentifié pour accéder aux objectifs nutritionnels',
        );
        return {
          success: false,
          error: 'Utilisateur non authentifié',
      ...
      if (!planResult?.success || !planResult.plan) {
        logger.error(
          LogCategory.NUTRITION,
          `Plan ${planId} non trouvé ou n'appartient pas à l'utilisateur`,
        );
        return {
          success: false,
          error:
            planResult?.error ||
            "Plan non trouvé ou n'appartient pas à l'utilisateur",
        };
      }
      ...
      if (
        ...
        plan.carbs === undefined ||
        plan.fat === undefined ||
        plan.protein === undefined
      ) {
        logger.error(
          LogCategory.NUTRITION,
          `Objectifs nutritionnels non définis pour le plan ${planId}`,
        );
        return {
          success: false,
      ...
      logger.debug(
        LogCategory.NUTRITION,
        `Format des valeurs nutritionnelles originales du plan ${planId}`,
        {
      ...
      if (areValuesInGrams) {
        ...
        const proteinGrams = Math.round(
          (plan.calories * (plan.protein / 100)) / 4,
        );
      ...
      logger.error(
        LogCategory.NUTRITION,
        `Erreur lors de la récupération des objectifs nutritionnels`,
        {
          error: error instanceof Error ? error.message : String(error),
          planId,
        },
