...
class MealPagesService implements MealPagesServiceInterface {
  ...
  async toggleMealFavorite(mealId: number, isFavorite: boolean): Promise<OperationResult<any>> {
    try {
      const result = await mealService.updateMealFavoriteStatus(mealId, isFavorite);
      if (!result.success) {
        return {
          success: false,
          error: result.error || 'Erreur lors de la mise à jour du statut favori',
        };
      }
      return { success: true };
    } catch (error) {
      logger.error(LogCategory.DATABASE, `Error in toggleMealFavorite service: ${error instanceof Error ? error.message : String(error)}`);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Une erreur est survenue',
      };
