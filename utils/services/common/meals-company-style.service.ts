...
class MealsCompanyStyleService {
  ...
  determineMealType(type: string | null): MealType {
    try {
      ...
      logger.error(LogCategory.APP, 'Error determining meal type', { error, type });
      return 'snacks';
    }
  }
    ...
    try {
      ...
      const updatePromises = movedItems.map(item => {
        if (!item.progress && !item.dailyPlanMealId) {
          logger.warn(LogCategory.APP, 'Missing progress info for meal', { item });
      ...
      logger.error(LogCategory.APP, 'Error updating meals status', { error, movedItems });
  ...
  toggleItemSelection(currentSelection: Set<number>, id: number): Set<number> {
    try {
      ...
      logger.error(LogCategory.APP, 'Error toggling item selection', { error, id });
