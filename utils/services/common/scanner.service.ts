...
class ScannerService {
    ...
    try {
      ...
      if (scanResult.isValid && scanResult.productResult) {
        this.cachedProducts.set(
          barcodeData,
          scanResult.productResult as ExtendedProductResult,
        );
      }

      return scanResult;
    } catch (error) {
      logger.error(LogCategory.INTEGRATION, 'Error scanning barcode', {
        error,
        barcodeData,
      });
      return {
  ...
  ): Promise<ExtendedProductResult[]> {
    try {
      logger.info(LogCategory.INTEGRATION, 'Searching products', {
        searchParams,
      });

      // Obtenir les produits de l'API et faire un cast vers ExtendedProductResult
      const apiProducts = await OpenFoodFactsService.searchProducts(
        searchParams,
      );
      ...
      logger.error(LogCategory.INTEGRATION, 'Error searching products', {
        error,
        searchParams,
      });
      throw new Error(
  ...
  private extractCalories(product: Product): number {
    ...
    return product.nutriments.energy_100g
  ...
  private extractNutrient(product: Product, nutrientKey: string): number {
    ...
    return (
      (product.nutriments[
        nutrientKey as keyof typeof product.nutriments
      ] as number) || 0
    );
  }
  ...
  getCuisineSuggestion(product: ExtendedProductResult): CuisineTypeEnum {
    try {
      ...
      if (
        origins.includes('italie') ||
        origins.includes('italy') ||
        origins.includes('italian')
      ) {
        return CuisineTypeEnum.ITALIAN;
      } else if (origins.includes('france') || origins.includes('french')) {
        return CuisineTypeEnum.FRENCH;
      } else if (
        origins.includes('mexique') ||
        origins.includes('mexico') ||
        origins.includes('mexican')
      ) {
        return CuisineTypeEnum.MEXICAN;
      } else if (
        origins.includes('japan') ||
        origins.includes('japon') ||
        origins.includes('japanese')
      ) {
        return CuisineTypeEnum.JAPANESE;
      } else if (
        origins.includes('chine') ||
        origins.includes('china') ||
        origins.includes('chinese')
      ) {
        return CuisineTypeEnum.CHINESE;
      } else if (
        origins.includes('united states') ||
        origins.includes('usa') ||
        origins.includes('american')
      ) {
      ...
      if (product.categories_tags) {
        ...
        if (
          ...
          categories.includes('italian')
        ) {
          return CuisineTypeEnum.ITALIAN;
        } else if (
          categories.includes('french') ||
          categories.includes('france')
        ) {
          return CuisineTypeEnum.FRENCH;
        } else if (
          categories.includes('mexican') ||
          categories.includes('taco') ||
          categories.includes('burrito')
        ) {
          return CuisineTypeEnum.MEXICAN;
        } else if (
          categories.includes('sushi') ||
          categories.includes('japanese')
        ) {
          return CuisineTypeEnum.JAPANESE;
        }
      }
      ...
      logger.error(LogCategory.APP, 'Error getting cuisine suggestion', {
        error,
        product,
      });
      return CuisineTypeEnum.GENERAL;
  ...
  formatNutrients(product: ExtendedProductResult): {
    ...
    try {
      ...
      logger.error(LogCategory.APP, 'Error formatting nutrients', {
        error,
        product,
      });
      return { calories: 0, protein: 0, fat: 0, carbs: 0 };
