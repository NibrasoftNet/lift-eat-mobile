...
class UserProfileValidatorService {
  ...
  validateProfileCompleteness(userDetails: User | UserDetails | null): { 
    ...
    logger.debug(LogCategory.USER, 'Vérification du profil utilisateur désactivée - onboarding obligatoire');
    ...
    if (!userDetails) {
      ...
      return {
        isComplete: false,
        missingItems: ['profil'],
        message: 'Veuillez vous connecter pour accéder à toutes les fonctionnalités.'
      };
    }
