...
export const mealService = {
   ...
   */
  async updateMealFavoriteStatus(mealId: number, isFavorite: boolean): Promise<{ success: boolean; error?: string }> {
    try {
      const userId = getCurrentUserIdSync();
      if (!userId) {
        logger.error(LogCategory.AUTH, 'Authentication required to update favorite status');
        return { success: false, error: 'You must be logged in to update a meal' };
      }

      logger.info(LogCategory.DATABASE, `Updating favorite status for meal ${mealId} to ${isFavorite}`);

      const result = await sqliteMCPServer.updateMealViaMCP(mealId, { isFavorite }, undefined, userId);
      if (!result.success) {
        logger.error(LogCategory.DATABASE, `Failed to update favorite status: ${result.error}`);
        return { success: false, error: result.error || 'Failed to update favorite status' };
      }
      return { success: true };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(LogCategory.DATABASE, `Error updating favorite status: ${errorMessage}`);
      return { success: false, error: errorMessage };
    }
  },
