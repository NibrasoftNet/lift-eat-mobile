...
export const assistantCoreService = {
  ...
  > {
    try {
      ...
      if (!result.mealId) {
        logger.error(
          LogCategory.IA,
          'ID du repas manquant dans le résultat de création',
        );
        return {
          success: false,
      ...
      if (!mealDetailsResult || !mealDetailsResult.success) {
        logger.warn(
          LogCategory.IA,
          'Repas généré mais échec de la récupération des détails',
          {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Une erreur est survenue lors de la génération du repas',
      };
    }
  },
  ...
  > {
    try {
      ...
      if (!result.planId) {
        logger.error(
          LogCategory.IA,
          'ID du plan manquant dans le résultat de création',
        );
        return {
          success: false,
      ...
      const planDetailsResult = await sqliteMCPServer.getPlanDetailsViaMCP(
        result.planId,
        userId,
      );
      ...
      if (!planDetailsResult || !planDetailsResult.success) {
        logger.warn(
          LogCategory.IA,
          'Plan généré mais échec de la récupération des détails',
          {
  ...
  > {
    try {
      logger.info(
        LogCategory.IA,
        "Génération d'une liste de courses pour le plan",
        {
          userId,
          planId,
        },
      ...
      return {
        ...
        data: {
          shoppingList: [
            ...
            {
              id: 3,
              name: 'Protéines (selon préférences)',
              quantity: 200,
              unit: 'g',
            },
          ],
      ...
      logger.error(
        LogCategory.IA,
        'Exception lors de la génération de la liste de courses',
        {
          error: error instanceof Error ? error.message : String(error),
          userId,
          planId,
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Une erreur est survenue lors de la génération de la liste de courses',
      };
    }
  },
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Une erreur est survenue lors de l'analyse des progrès",
      };
    }
  },
   ...
   */
  async analyzeNutritionHabits(userId: number): Promise<
    OperationResult<{
      analysis: {
        text: string;
        recommendations: string[];
  ...
  > {
    try {
      logger.info(
        LogCategory.IA,
        "Analyse des habitudes nutritionnelles via l'IA",
        { userId },
      );
      ...
      return {
        ...
        data: {
          analysis: {
            ...
            improvements: ['Trop de sucres ajoutés', 'Pas assez de fibres'],
          },
        },
      ...
      logger.error(
        LogCategory.IA,
        "Erreur lors de l'analyse des habitudes nutritionnelles",
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Une erreur est survenue lors de l'analyse des habitudes nutritionnelles",
      };
    }
  },
};
