...
export const progressService = {
  ...
  async markMealAsConsumed(
    ...
    try {
      logger.info(
        ...
        "Mise à jour du statut de consommation d'un repas",
        {
          dailyProgressId,
          mealId,
          dailyPlanMealId,
      ...
      if (!result.success) {
        throw new Error(
          result.error || 'Erreur lors de la mise à jour du repas',
        );
      }
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la mise à jour du statut de consommation: ${errorMessage}`,
        {
          dailyProgressId,
          mealId,
          consumed,
   ...
   */
  async getDailyProgressByDate(date: string): Promise<{
    success: boolean;
    error?: string;
    ...
    try {
      ...
      if (!userId) {
        logger.error(
          LogCategory.AUTH,
          'Authentification requise pour récupérer la progression quotidienne',
        );
        return {
          success: false,
          error: 'Vous devez être connecté pour récupérer votre progression',
        };
      }

      logger.info(
        LogCategory.DATABASE,
        `Récupération de la progression quotidienne pour ${date}`,
      );
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la récupération de la progression: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  },
   ...
   */
  async getMealsForDailyProgress(dailyProgressId: number): Promise<{
    success: boolean;
    error?: string;
    ...
    try {
      logger.info(
        LogCategory.DATABASE,
        `Récupération des repas pour la progression quotidienne ${dailyProgressId}`,
      );

      return await sqliteMCPServer.getMealProgressByDailyProgressViaMCP(
        dailyProgressId,
      );
    } catch (error) {
      const errorMessage =
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la récupération des repas pour la progression: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  },
   ...
   */
  async getProgressionsByPlan(planId: number): Promise<{
    success: boolean;
    error?: string;
    ...
    try {
      ...
      if (!userId) {
        logger.error(
          LogCategory.AUTH,
          'Authentification requise pour récupérer les progressions',
        );
        return {
          success: false,
          error: 'Vous devez être connecté pour récupérer vos progressions',
        };
      }

      logger.info(
        LogCategory.DATABASE,
        `Récupération des progressions associées au plan ${planId}`,
      );
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la récupération des progressions: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  },
  ...
  async createDailyProgress(date: string): Promise<{
    ...
    try {
      ...
      if (!userId) {
        logger.error(
          LogCategory.AUTH,
          'Authentification requise pour créer une progression',
        );
        return {
          success: false,
          error: 'Vous devez être connecté pour enregistrer votre progression',
        };
      }

      logger.info(
        LogCategory.DATABASE,
        `Création de la progression quotidienne pour ${date}`,
      );
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la création de la progression: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  },
  ...
  async updateDailyProgress(
    ...
    try {
      logger.info(
        LogCategory.DATABASE,
        `Mise à jour de la progression quotidienne ${progressId}`,
      );
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la mise à jour de la progression: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  },
   ...
   */
  async getMealProgressByDate(date: string): Promise<{
    success: boolean;
    error?: string;
    meals?: any[];
  }> {
    try {
      ...
      if (!userId) {
        logger.error(
          LogCategory.AUTH,
          'Authentification requise pour récupérer la progression des repas',
        );
        return {
          success: false,
          error: 'Vous devez être connecté pour récupérer votre progression',
      ...
      logger.info(
        LogCategory.DATABASE,
        `Récupération de la progression des repas pour ${date}`,
      );
      ...
      logger.error(
        LogCategory.DATABASE,
        `Erreur lors de la récupération de la progression des repas: ${errorMessage}`,
      );
      return { success: false, error: errorMessage };
    }
  },
  ...
  async getProgressHistory(
    ...
    try {
      ...
      if (!userId) {
        logger.error(
          LogCategory.AUTH,
          "Authentification requise pour récupérer l'historique de progression",
        );
        return {
          success: false,
          error: 'Vous devez être connecté pour récupérer votre progression',
      ...
      logger.info(
        LogCategory.DATABASE,
        "Récupération de l'historique de progression",
        {
          userId,
          startDate,
          endDate,
      ...
      if (days > 31) {
        logger.warn(
          LogCategory.DATABASE,
          'Plage de dates trop grande, limitation à 31 jours',
        );
      }
...
export const markMealAsConsumed = async (
  ...
  logger.warn(
    LogCategory.DEPRECATED,
    'La fonction markMealAsConsumed est dépréciée, utilisez progressService.markMealAsConsumed à la place',
  );
