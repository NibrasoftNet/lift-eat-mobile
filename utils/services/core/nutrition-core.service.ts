...
import { roundToDecimals, preciseCalculation } from '@/utils/helpers/precision.helper';
import { 
  NutritionLimits, 
  MacroCalorieFactors,
  NutritionRoundingPolicy
} from '@/utils/constants/NutritionConstants';
import { CookingMethod, CookingMethodFactors } from '@/utils/constants/CookingConstants';
import { nutritionEngine } from '@/utils/engines/nutrition-engine';
import { NutritionDisplayMode, getNormalizationFactorForMode, getDisplayTextForMode } from '@/utils/enum/nutrition.enum';
...
class NutritionCoreService {
  ...
  normalizeNutritionalValues({
    ...
    try {
      ...
      if (displayMode === NutritionDisplayMode.PER_SERVING && (!servingSize || servingSize <= 0)) {
        logger.warn(
          LogCategory.NUTRITION,
      ...
      const displayFactor = getNormalizationFactorForMode(displayMode, totalWeight, servingSize);
      ...
      const displayText = getDisplayTextForMode(
        displayMode, 
        displayMode === NutritionDisplayMode.PER_SERVING ? servingSize : totalWeight
      );
   ...
   */
  getValuesByQuantity(macros: MacroNutrientsBase, standardWeight: number, targetQuantity: number) {
    const mode = targetQuantity === standardWeight 
  ...
  isValidWeight(weight: number): boolean {
    return this.isValidNutritionalValue(weight) && weight >= NutritionLimits.WEIGHT.MIN;
  }
  ...
  isValidCalories(calories: number): boolean {
    return this.isValidNutritionalValue(calories) && calories <= NutritionLimits.CALORIES.MAX;
  }
  ...
  isValidMacro(value: number): boolean {
    return this.isValidNutritionalValue(value) && value <= NutritionLimits.MACROS.MAX;
  }
  ...
  validateNutritionalValues(
    ...
    if (!this.isValidNutritionalValue(calories) || 
        !this.isValidNutritionalValue(carbs) || 
        !this.isValidNutritionalValue(fat) || 
        !this.isValidNutritionalValue(protein)) {
      return { valid: false, reason: 'Valeurs nutritionnelles invalides (nu00e9gatives ou non numu00e9riques)' };
    }
    
    // Vu00e9rifier les bornes
    if (!this.isValidCalories(calories)) {
      return { valid: false, reason: `Calories hors limites (max: ${NutritionLimits.CALORIES.MAX})` };
    }
    
    if (!this.isValidMacro(carbs)) {
      return { valid: false, reason: `Glucides hors limites (max: ${NutritionLimits.MACROS.MAX})` };
    }
    
    if (!this.isValidMacro(fat)) {
      return { valid: false, reason: `Lipides hors limites (max: ${NutritionLimits.MACROS.MAX})` };
    }
    
    if (!this.isValidMacro(protein)) {
      return { valid: false, reason: `Protu00e9ines hors limites (max: ${NutritionLimits.MACROS.MAX})` };
    }
    
    // Vu00e9rifier le sucre si fourni
    if (sugar !== undefined && !this.isValidMacro(sugar)) {
      return { valid: false, reason: `Sucres hors limites (max: ${NutritionLimits.MACROS.MAX})` };
    }
    
    // Vu00e9rifier la cohu00e9rence des calories
    const calculatedCalories = this.calculateCaloriesFromMacros(carbs, protein, fat);
    ...
    if (Math.abs(calculatedCalories - calories) > margin) {
      return { 
        valid: false, 
        reason: `Incohu00e9rence calories: ${calories} kcal du00e9claru00e9es vs ${Math.round(calculatedCalories)} kcal calculu00e9es` 
      };
    }
   ...
   */
  calculateCaloriesFromMacros(carbs: number, protein: number, fat: number): number {
    return preciseCalculation(
      (c, p, f) => c * MacroCalorieFactors.CARBS + 
  ...
  calculateMacrosFromCaloriesAndDistribution(
    ...
    try {
      ...
      const protein = Math.round((calories * proteinRatio) / MacroCalorieFactors.PROTEIN);
      
      // Glucides : 4 calories par gramme
      const carbs = Math.round((calories * carbsRatio) / MacroCalorieFactors.CARBS);
      
      // Lipides : 9 calories par gramme
      const fat = Math.round((calories * effectiveFatRatio) / MacroCalorieFactors.FAT);
      
      // Recalculer les calories exactes u00e0 partir des grammes pour plus de pru00e9cision
      const recalculatedCalories = this.calculateCaloriesFromMacros(carbs, protein, fat);
      ...
      logger.error(LogCategory.NUTRITION, 'Erreur dans calculateMacrosFromCaloriesAndDistribution', { error });
  ...
  calculateTotalMacros(meals: any[]) {
    try {
      ...
      logger.error(LogCategory.NUTRITION, 'Erreur dans calculateTotalMacros:', { error });
  ...
  calculateProportionalMacros(
    ...
    if (!this.isValidWeight(standardQuantity) || !this.isValidWeight(newQuantity)) {
    ...
    if (!this.isValidMacro(newCarbs) || !this.isValidMacro(newFat) || !this.isValidMacro(newProtein)) {
  ...
  adjustMacrosByCookingMethod(
    ...
    try {
      ...
      logger.error(LogCategory.NUTRITION, 'Erreur dans adjustMacrosByCookingMethod', { error });
  ...
  adjustMacrosByFinalWeight(
    macros: { calories: number; carbs: number; fat: number; protein: number, sugar?: number },
    ...
    if (totalIngredientsWeight < NutritionLimits.WEIGHT.MIN || finalMealWeight < NutritionLimits.WEIGHT.MIN) {
      return {
        calories: 0,
        carbs: 0,
        fat: 0,
    ...
    try {
      ...
      const adjustedMacros = {
        ...
        sugar: macros.sugar !== undefined ? Math.round(macros.sugar * adjustmentFactor) : undefined
      };
      ...
      if (!this.isValidMacro(adjustedMacros.carbs) || 
          !this.isValidMacro(adjustedMacros.fat) || 
          !this.isValidMacro(adjustedMacros.protein) ||
          (adjustedMacros.sugar !== undefined && !this.isValidMacro(adjustedMacros.sugar))) {
        return macros; // Retourner les macros originales si les macros ajustu00e9es sont invalides
      }

      return adjustedMacros;
    } catch (error) {
      logger.error(LogCategory.NUTRITION, "Erreur lors de l'ajustement des macros:", error);
  ...
  calculateCaloriesIntake(userProfile: {
    ...
    try {
      logger.info(LogCategory.NUTRITION, 'Calcul des besoins caloriques', userProfile);
      ...
      logger.info(LogCategory.NUTRITION, `Besoins caloriques calculu00e9s: ${dailyCalories} kcal/jour`);
      
      return dailyCalories;
    } catch (error) {
      logger.error(LogCategory.NUTRITION, 'Erreur lors du calcul des besoins caloriques', {
  ...
  async calculateMealNutrition(
    mealOrId: number | { id: number; standardQuantity: number; calories: number; carbs: number; fat: number; protein: number; },
    quantity: number,
    ...
    try {
      ...
      logger.info(LogCategory.NUTRITION, `Calcul nutritionnel pour le repas ${mealId} (quantité: ${quantity}g, mode: ${displayMode})`);
      const result = await sqliteMCPServer.calculateMealNutritionViaMCP(mealId, quantity);
      ...
      if (result && result.success && result.nutrition) {
        ...
        if (displayMode !== NutritionDisplayMode.AS_IS) {
          ...
          const normalizedNutrition = normalizeMacrosToReferenceWeight(
            rawMacros, 
            quantity, 
            displayMode === NutritionDisplayMode.PER_SERVING ? (servingSize || 100) : 100,
            displayMode
          );
      ...
      if (typeof mealOrId === 'object') {
        ...
        logger.warn(LogCategory.NUTRITION, 'Fallback sur le helper local pour le calcul nutritionnel', { 
          mealId: meal.id, 
          quantity,
          displayMode
        });
      ...
      logger.error(LogCategory.NUTRITION, `Erreur calcul nutritionnel: ${errorMessage}`, {
        mealOrId, 
        quantity,
        displayMode
      });
  ...
  async calculateMacroBreakdown(
    ...
    try {
      const { userId, displayMode = NutritionDisplayMode.PER_100G, quantity } = options;
      
      logger.info(LogCategory.NUTRITION, `Calcul de la répartition des macros pour le repas ${mealId}`);
      ...
      if (!nutritionResult.success) {
        return {
          success: false,
          error: nutritionResult.error || "Impossible de calculer les valeurs nutritionnelles",
          calories: 0,
          carbs: 0,
          fat: 0,
          protein: 0,
      ...
      const proteinCalories = nutritionResult.protein * MacroCalorieFactors.PROTEIN;
      ...
      const totalCalories = nutritionResult.calories || (proteinCalories + carbsCalories + fatCalories);
      ...
      if (totalPercentage !== 100) {
        ...
        adjustedProteinPercentage = Math.round((proteinPercentage / totalPercentage) * 100);
        adjustedCarbsPercentage = Math.round((carbsPercentage / totalPercentage) * 100);
        adjustedFatPercentage = 100 - adjustedProteinPercentage - adjustedCarbsPercentage;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(LogCategory.NUTRITION, `Erreur lors du calcul de la répartition des macros: ${errorMessage}`);
      return {
        success: false,
        error: errorMessage,
        calories: 0,
  ...
  async calculateDailyPlanNutrition(
    ...
    try {
      logger.info(LogCategory.NUTRITION, `Calcul des valeurs nutritionnelles pour le plan journalier ${dailyPlanId}`);
      ...
      if (!result.success || !result.nutrition) {
        const errorMsg = result.error || "Impossible de récupérer les valeurs nutritionnelles";
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(LogCategory.NUTRITION, `Erreur lors du calcul nutritionnel du plan journalier: ${errorMessage}`);
      return { 
        success: false, 
        error: errorMessage,
        calories: 0,
   ...
   */
  calculateNutritionProgress(currentValues: MacroNutrientsBase, goalValues: MacroNutrientsBase): NutritionProgress {
    try {
      // Calculer les pourcentages pour chaque nutriment
      const percentages = {
        calories: goalValues.calories > 0 ? (currentValues.calories / goalValues.calories) * 100 : 0,
        carbs: goalValues.carbs > 0 ? (currentValues.carbs / goalValues.carbs) * 100 : 0,
        protein: goalValues.protein > 0 ? (currentValues.protein / goalValues.protein) * 100 : 0,
        fat: goalValues.fat > 0 ? (currentValues.fat / goalValues.fat) * 100 : 0
      };
      ...
      logger.error(LogCategory.NUTRITION, 'Erreur lors du calcul des progrès nutritionnels', {
        error,
        currentValues,
        goalValues
      });
  ...
  analyzeDayNutrition(meals: any[]) {
    try {
      ...
      const validation = meals.length > 0
      ...
      logger.error(LogCategory.NUTRITION, 'Erreur lors de l\'analyse nutritionnelle de la journu00e9e', { meals });
      throw error;
    }
  }
  ...
  updateUserNutritionPreferences(userId: number, preferences: any) {
    try {
      logger.info(LogCategory.NUTRITION, 'Mise à jour des préférences nutritionnelles', { userId, preferences });
      return sqliteMCPServer.updateUserPreferencesViaMCP(userId, preferences);
    } catch (error) {
      logger.error(LogCategory.NUTRITION, 'Erreur lors de la mise à jour des préférences nutritionnelles', { 
        error: error instanceof Error ? error.message : String(error), 
        userId, 
        preferences 
      });
  ...
  async createPlan(planData: any, userId: number): Promise<OperationResult<{planId?: number}>> {
    try {
      logger.info(LogCategory.NUTRITION, 'Création d\'un nouveau plan nutritionnel', { userId });
      
      const result = await sqliteMCPServer.createPlanViaMCP(planData, userId);
      
      if (!result.success) {
        logger.error(LogCategory.NUTRITION, `Échec de création du plan nutritionnel via MCP: ${result.error}`, { userId });
        return {
          success: false,
      ...
      if (!result.planId) {
        logger.error(LogCategory.NUTRITION, 'Aucun ID de plan retourné par le serveur MCP', { userId });
        return {
          success: false,
      ...
      logger.info(LogCategory.NUTRITION, 'Plan nutritionnel créé avec succès', { userId, planId: result.planId });
      return {
        success: true,
        data: { planId: result.planId }
      };
    } catch (error) {
      logger.error(LogCategory.NUTRITION, 'Exception lors de la création du plan nutritionnel', {
        userId,
        error: error instanceof Error ? error.message : String(error)
      });
      return {
  ...
  normalizeIngredientsTo100g(ingredients: any[], totalWeight: number) {
    try {
      ...
      if (!ingredients || ingredients.length === 0) {
        logger.warn(LogCategory.NUTRITION, 'Normalisation impossible: aucun ingrédient fourni');
        return {
      ...
      if (totalWeight <= 0) {
        logger.warn(LogCategory.NUTRITION, `Poids total invalide: ${totalWeight}g, impossible de normaliser`);
        return {
      ...
      if (Math.abs(totalWeight - STANDARD_WEIGHT) < 0.01) {
        logger.info(LogCategory.NUTRITION, `Poids déjà à ${STANDARD_WEIGHT}g, pas de normalisation nécessaire`);
        return {
          normalizedIngredients: [...ingredients],
          normalizationFactor: 1,
          totalWeightBefore: totalWeight,
      ...
      logger.info(
        LogCategory.NUTRITION, 
        `Facteur de normalisation: ${normalizationFactor.toFixed(4)}x (${totalWeight}g → ${STANDARD_WEIGHT}g)`
      );
      ...
      logger.error(
        LogCategory.NUTRITION, 
        `Erreur lors de la normalisation des ingrédients: ${error instanceof Error ? error.message : String(error)}`,
  ...
  getDefaultNutritionGoals() {
    logger.debug(LogCategory.NUTRITION, 'Récupération des objectifs nutritionnels par défaut');
    return {
  ...
  validateMacroNutrientPercentages(proteinPercentage: number, carbsPercentage: number, fatPercentage: number): string | null {
    logger.debug(LogCategory.NUTRITION, `Validation des macronutriments: P:${proteinPercentage}%, C:${carbsPercentage}%, F:${fatPercentage}%`);
  ...
  async calculateRecommendedNutritionGoals(userId: number) {
    try {
      logger.info(LogCategory.NUTRITION, `Calcul des objectifs nutritionnels pour l'utilisateur ${userId}`);
      ...
      if (!userDetailsResult.success || !userDetailsResult.user) {
        logger.error(LogCategory.NUTRITION, `Impossible de récupérer le profil utilisateur ${userId}`);
      ...
      const userProfile = {
        ...
        physicalActivity: this.mapActivityLevelToCalculationFormat(userDetails.physicalActivity || 'MODERATE')
      };
      
      // Calcul des besoins caloriques quotidiens basés sur le profil
      const dailyCalories = this.calculateCaloriesIntake(userProfile);
      
      logger.info(LogCategory.NUTRITION, `Besoins caloriques calculés pour l'utilisateur ${userId}: ${dailyCalories} kcal/jour`);
      ...
      logger.error(LogCategory.NUTRITION, `Erreur lors du calcul des objectifs nutritionnels: ${error instanceof Error ? error.message : String(error)}`);
      return this.getDefaultNutritionGoals();
    }
  }
