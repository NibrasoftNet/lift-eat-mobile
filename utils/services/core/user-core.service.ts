...
class UserCoreService implements UserPagesServiceInterface {
   ...
   */
  async validateUserExists(id: number): Promise<OperationResult<{exists: boolean}>> {
    try {
      logger.info(LogCategory.DATABASE, 'Vérification de l\'existence de l\'utilisateur', { id });
      
      const result = await sqliteMCPServer.validateUserExistsViaMCP(id);
      
      return result;
    } catch (error) {
      logger.error(LogCategory.DATABASE, 'Erreur lors de la vérification de l\'existence de l\'utilisateur', {
        error: error instanceof Error ? error.message : String(error)
      });
      return {
        success: false,
   ...
   */
  async createUser(userData: any): Promise<OperationResult<{userId: number}>> {
    try {
      ...
      logger.error(LogCategory.DATABASE, 'Erreur lors de la création de l\'utilisateur', {
        error: error instanceof Error ? error.message : String(error)
      });
      return {
        success: false,
  ...
  validateWeightUnit(unit: any): WeightUnitEnum {
    try {
      ...
      logger.warn(LogCategory.FORM, 'Invalid weight unit, using default', { unit });
      return WeightUnitEnum.KG;
    } catch (error) {
  ...
  validateHeightUnit(unit: any): HeightUnitEnum {
    try {
      ...
      // Si aucune correspondance n'est trouvée, utiliser la valeur par défaut
      logger.warn(LogCategory.FORM, 'Invalid height unit, using default', { unit });
      return HeightUnitEnum.CM;
    } catch (error) {
  ...
  async updateUserProfileData(id: number, profileData: {
    ...
    try {
      ...
      if (!userExists.success || !userExists.exists) {
        logger.error(LogCategory.DATABASE, `Utilisateur avec ID ${id} introuvable`);
        return {
          success: false,
      ...
      logger.debug(LogCategory.DATABASE, 'Données du profil à mettre à jour', { profileData });
      ...
      logger.debug(LogCategory.DATABASE, 'Données valides du profil', { validUserData });
      
      // Mettre à jour les données utilisateur avec les données valides
      const updateResult = await sqliteMCPServer.updateUserPreferencesViaMCP(id, validUserData);
