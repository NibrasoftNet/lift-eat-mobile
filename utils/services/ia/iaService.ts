...
import { validateMealWithRecovery, validatePlanWithRecovery } from './responseValidation';
...
export const iaService = {
    ...
    try {
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          if (retryCount > 0) {
            logger.info(LogCategory.IA, `iaService.request: Tentative ${retryCount}/${maxRetries} après une erreur`);
            await new Promise(resolve => setTimeout(resolve, delayMs));
            delayMs *= BACKOFF_FACTOR; // Backoff exponentiel
          }
          
          // Appel au fournisseur IA
          const providerResponse = await iaProvider.sendRequest(enrichedPrompt, requestOptions);
          ...
          if (action.type !== 'NONE') {
            ...
            if (action.isValid) {
              // Traiter l'action DB
              const processingResult = await processDatabaseAction(action, userId);
              
              logger.info(LogCategory.IA, `iaService.request: Action ${action.type} traitée avec succès${action.hadRecovery ? ' (avec récupération)' : ''}`);
              ...
              // Action invalide
              logger.warn(LogCategory.IA, `iaService.request: Action ${action.type} invalidée: ${action.validationMessage || 'Raison inconnue'}`);
              
              // Si on a atteint le nombre maximum de tentatives, retourner l'erreur
              if (retryCount >= maxRetries) {
                return {
                  success: false,
                  error: `Action invalidée: ${action.validationMessage || 'Raison inconnue'}`,
                  cleanText,
                  retryCount
                };
              }
            ...
            logger.info(LogCategory.IA, `iaService.request: Réponse textuelle reçue (${response.length} caractères)`);
          ...
          if (isRetryable) {
            retryCount++;
            logger.warn(LogCategory.IA, `iaService.request: Erreur récupérable, nouvelle tentative ${retryCount}/${maxRetries}`);
            continue;
          } else {
            logger.error(LogCategory.IA, `iaService.request: Erreur non récupérable`, error);
            throw error;
          }
        }
      }
      ...
      logger.error(LogCategory.IA, `iaService.request: Nombre maximum de tentatives dépassé (${maxRetries})`);
  ...
  async callGeminiApi(prompt: string): Promise<string> {
    try {
      ...
      const geminiProvider = iaProviderFactory.createProvider(IaProviderType.GEMINI);
      ...
      throw new IaError(
        `Erreur lors de l'appel à l'API Gemini: ${error instanceof Error ? error.message : String(error)}`,
        IaErrorType.API_ERROR,
        error
      );
    }
