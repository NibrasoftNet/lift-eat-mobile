...
const JSON_PATTERNS = {
  ...
  COMMON_TYPOS: [
    ...
    { search: /"(\w+)"\s*:\s*([^,}\]"]+)\s*([,}\]])/g, replace: '"$1": "$2"$3' }, // Met des guillemets autour des valeurs string sans guillemets
...
export function extractAndCleanJSON(text: string): string {
  ...
  try {
    ...
    logger.debug(LogCategory.IA, `extractAndCleanJSON: Tentative d'extraction de JSON invalide`);
  }
  ...
  if (markdownMatch && markdownMatch[1]) {
    ...
    try {
      ...
      logger.debug(LogCategory.IA, `extractAndCleanJSON: JSON valide extrait d'un bloc de code markdown`);
      return codeContent;
    } catch (mdError) {
      // Si le bloc n'est pas un JSON valide, on continue avec ce contenu
      logger.debug(LogCategory.IA, `extractAndCleanJSON: Bloc de code markdown trouvé mais JSON invalide: ${mdError}`);
      text = codeContent;
    }
  }
  ...
  const objectMatch = text.match(new RegExp(`${JSON_PATTERNS.OBJECT_START.source}[\\s\\S]*?\\}(?=[^}]|$)`));
  const arrayMatch = text.match(new RegExp(`${JSON_PATTERNS.ARRAY_START.source}[\\s\\S]*?\\](?=[^\\]]|$)`));

  if (objectMatch) {
    result = objectMatch[0];
    logger.debug(LogCategory.IA, `extractAndCleanJSON: Structure d'objet JSON détectée`);
  } else if (arrayMatch) {
    result = arrayMatch[0];
    logger.debug(LogCategory.IA, `extractAndCleanJSON: Structure de tableau JSON détectée`);
  } else {
    ...
    if (potentialObjectStart !== -1 && potentialObjectEnd !== -1 && potentialObjectStart < potentialObjectEnd) {
      result = text.substring(potentialObjectStart, potentialObjectEnd + 1);
      logger.debug(LogCategory.IA, `extractAndCleanJSON: Extraction potentielle d'un objet JSON par délimiteurs`);
    } else {
      logger.warn(LogCategory.IA, `extractAndCleanJSON: Impossible de détecter une structure JSON`);
      return '';
    }
  }
  ...
  result = result.replace(/\\t/g, '\\t').replace(/\\r/g, '\\r').replace(/\\n/g, '\\n');

  // 3. Détection et correction du préfixe "Object" (pour l'erreur "Unexpected character: O")
  if (result.startsWith('Object')) {
    logger.debug(LogCategory.IA, `extractAndCleanJSON: Détection du préfixe 'Object', tentative de correction`);
  ...
  try {
    JSON.parse(result);
    logger.debug(LogCategory.IA, `extractAndCleanJSON: Nettoyage réussi, JSON valide obtenu`);
    return result;
  } catch (e) {
    ...
    try {
      ...
      if (e instanceof SyntaxError && e.message.includes('Unexpected end of input')) {
        ...
        if (openBraces > closeBraces) {
          ...
          logger.debug(LogCategory.IA, `extractAndCleanJSON: Ajout de ${missingBraces} accolades fermantes manquantes`);
        }
        ...
        if (openBrackets > closeBrackets) {
          ...
          logger.debug(LogCategory.IA, `extractAndCleanJSON: Ajout de ${missingBrackets} crochets fermants manquants`);
        }

        // Essayer d'analyser à nouveau
        JSON.parse(result);
        logger.debug(LogCategory.IA, `extractAndCleanJSON: Correction des délimiteurs réussie`);
        return result;
      }
      ...
      if (e instanceof SyntaxError) {
        ...
        JSON.parse(result);
        logger.debug(LogCategory.IA, `extractAndCleanJSON: Correction de la syntaxe réussie`);
        return result;
      }
    } catch (finalError) {
      // Si toutes les tentatives de correction échouent
      logger.warn(LogCategory.IA, `extractAndCleanJSON: Impossible d'obtenir un JSON valide après corrections avancées: ${finalError}`);
      ...
      try {
        ...
        if (nameMatch && nameMatch[1] && nameMatch[1].trim() !== ',' && nameMatch[1].length > 1) {
        ...
        let mealType = "DINNER";
        if (typeMatch && typeMatch[1] && Object.values(MealTypeEnum).includes(typeMatch[1] as any)) {
        ...
        const minimalValid = {
          ...
          instructions: "Instructions non disponibles suite à une erreur de génération.",
          ingredients: [],
          cuisine: "GENERAL",
          calories: 0,
          carbs: 0,
        ...
        logger.warn(LogCategory.IA, `extractAndCleanJSON: Création d'un objet minimal pour éviter l'échec total avec nom: ${mealName}`);
        return JSON.stringify(minimalValid);
      } catch (lastError) {
        // Si même cela échoue, nous devons abandonner
        logger.error(LogCategory.IA, `extractAndCleanJSON: Échec de la création d'un objet minimal: ${lastError}`);
      }
 ...
 */
export function validateIngredientWithRecovery(data: string): EnhancedValidationResult<IaIngredientType> {
  try {
    ...
    if (!cleanedData) {
      return {
        success: false,
        message: "Impossible d'extraire des données JSON valides pour l'ingrédient",
        hadRecovery: false
      };
    }
    ...
    logger.info(LogCategory.IA, `validateIngredientWithRecovery: Tentative de récupération pour "${standardValidation.message}"`);
    ...
    try {
      ...
      return {
        success: false,
        message: `Erreur d'analyse JSON: ${e instanceof Error ? e.message : String(e)}`,
        hadRecovery: false
      };
    }
    ...
    if (!jsonData.name || typeof jsonData.name !== 'string' || jsonData.name.trim() === '') {
      return {
        success: false,
        message: "Le nom de l'ingrédient est requis et doit être une chaîne de caractères non vide",
        hadRecovery: false
      };
    }
    ...
    if (!jsonData.unit || typeof jsonData.unit !== 'string') {
      ...
      recoveryActions.push(`Unité manquante ou invalide, défaut utilisé: ${jsonData.unit}`);
    } else {
      // Normaliser l'unité
    ...
    // Quantité
    if (typeof jsonData.quantity !== 'number' || isNaN(jsonData.quantity) || jsonData.quantity <= 0) {
      jsonData.quantity = 100;
      recoveryActions.push(`Quantité invalide, défaut utilisé: ${jsonData.quantity}`);
    }

    // Valeurs nutritionnelles
    ['calories', 'carbs', 'protein', 'fat'].forEach(nutrient => {
      if (typeof jsonData[nutrient] !== 'number' || isNaN(jsonData[nutrient]) || jsonData[nutrient] < 0) {
        jsonData[nutrient] = 0;
    ...
    if (recoveryValidation.success) {
      logger.info(LogCategory.IA, `validateIngredientWithRecovery: Récupération réussie pour l'ingrédient "${jsonData.name}"`);
      return {
        ...recoveryValidation,
        hadRecovery: true,
        recoveryActions
    ...
    logger.error(LogCategory.IA, `validateIngredientWithRecovery: Erreur fatale: ${errorMessage}`);
 ...
 * @returns Résultat de validation renforcée
 */
export function validateMealWithRecovery(data: string): EnhancedValidationResult<IaMealType> {
  try {
    // 1. Nettoyer et extraire le JSON
    const cleanedData = extractAndCleanJSON(data);
    if (!cleanedData) {
    ...
    // 3. Tentative de récupération
    logger.info(LogCategory.IA, `validateMealWithRecovery: Tentative de récupération pour "${standardValidation.message}"`);
    ...
    if (!jsonData.name || typeof jsonData.name !== 'string' || jsonData.name.trim() === '') {
      return {
        success: false,
        message: "Le nom du repas est requis et doit être une chaîne de caractères non vide",
        hadRecovery: false
      };
    }

    // Type de repas
    if (!jsonData.type || !Object.values(MealTypeEnum).includes(jsonData.type)) {
      ...
      if (mealName.includes("petit") || mealName.includes("déjeuner") || mealName.includes("matin")) {
        defaultType = MealTypeEnum.BREAKFAST;
      } else if (mealName.includes("déjeuner") || mealName.includes("midi") || mealName.includes("lunch")) {
        defaultType = MealTypeEnum.LUNCH;
      } else if (mealName.includes("dîner") || mealName.includes("soir") || mealName.includes("dinner")) {
        defaultType = MealTypeEnum.DINNER;
      } else if (mealName.includes("collation") || mealName.includes("snack") || mealName.includes("goûter")) {
        defaultType = MealTypeEnum.SNACK;
      }
      
      jsonData.type = defaultType;
      recoveryActions.push(`Type de repas manquant ou invalide, défaut utilisé: ${jsonData.type}`);
    }

    // Description
    if (!jsonData.description || typeof jsonData.description !== 'string') {
      jsonData.description = "";
      recoveryActions.push("Description manquante ou invalide, chaîne vide utilisée");
    }
    ...
    if (!jsonData.instructions || typeof jsonData.instructions !== 'string') {
      ...
      if (jsonData.description && jsonData.description.length > 100) {
        ...
        recoveryActions.push("Instructions extraites à partir de la description");
      } else {
        jsonData.instructions = "";
        recoveryActions.push("Instructions manquantes ou invalides, chaîne vide utilisée");
      }
    }

    // Cuisine
    if (!jsonData.cuisine || !Object.values(CuisineTypeEnum).includes(jsonData.cuisine)) {
      if (typeof jsonData.cuisine === 'string') {
        // Normaliser la cuisine
        jsonData.cuisine = normalizeCuisineType(jsonData.cuisine);
        recoveryActions.push(`Type de cuisine normalisé: "${jsonData.cuisine}"`);
      } else {
        jsonData.cuisine = CuisineTypeEnum.GENERAL;
        recoveryActions.push(`Type de cuisine manquant ou invalide, défaut utilisé: ${jsonData.cuisine}`);
      }
    }

    // Unité
    if (!jsonData.unit || !Object.values(MealUnitEnum).includes(jsonData.unit)) {
    ...
    if (!jsonData.ingredients || !Array.isArray(jsonData.ingredients)) {
      ...
      recoveryActions.push("Liste d'ingrédients manquante ou invalide, liste vide utilisée");
    } else {
      ...
      for (const ingredient of jsonData.ingredients) {
        if (typeof ingredient === 'object' && ingredient !== null) {
          ...
          const ingredientValidation = validateIngredientWithRecovery(JSON.stringify(ingredient));
          ...
          if (ingredientValidation.success) {
            ...
            recoveryActions.push(`Ingrédient ignoré: ${ingredientValidation.message}`);
          }
        }
      }
    ...
    if (recoveryValidation.success) {
      logger.info(LogCategory.IA, `validateMealWithRecovery: Récupération réussie pour le repas "${jsonData.name}"`);
      return {
        ...recoveryValidation,
        hadRecovery: true,
        recoveryActions
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error(LogCategory.IA, `validateMealWithRecovery: Erreur fatale: ${errorMessage}`);
 ...
 * @returns Résultat de validation renforcée
 */
export function validatePlanWithRecovery(data: string): EnhancedValidationResult<IaPlanType> {
  try {
    // 1. Nettoyer et extraire le JSON
    const cleanedData = extractAndCleanJSON(data);
    if (!cleanedData) {
    ...
    // 3. Tentative de récupération
    logger.info(LogCategory.IA, `validatePlanWithRecovery: Tentative de récupération pour "${standardValidation.message}"`);
    ...
    if (!jsonData.name || typeof jsonData.name !== 'string' || jsonData.name.trim() === '') {
      ...
      recoveryActions.push(`Nom manquant ou invalide, défaut utilisé: "${jsonData.name}"`);
    }

    // Goal (requis par le schéma)
    if (!jsonData.goal) {
      jsonData.goal = "GAIN_MUSCLE"; // Valeur par défaut
      recoveryActions.push(`Objectif manquant, défaut utilisé: ${jsonData.goal}`);
    }
    ...
    if (!jsonData.meals || !Array.isArray(jsonData.meals) || jsonData.meals.length === 0) {
      ...
      recoveryActions.push("Liste de repas manquante ou vide, repas par défaut ajouté");
    } else {
    ...
    if (recoveryValidation.success) {
      logger.info(LogCategory.IA, `validatePlanWithRecovery: Récupération réussie pour le plan "${jsonData.name}"`);
      return {
        ...recoveryValidation,
        hadRecovery: true,
        recoveryActions
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error(LogCategory.IA, `validatePlanWithRecovery: Erreur fatale: ${errorMessage}`);
