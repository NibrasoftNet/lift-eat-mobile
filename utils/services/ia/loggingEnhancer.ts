...
export class IaLogger {
  ...
  private redactSensitiveData(obj: any): void {
    ...
    for (const key in obj) {
      if (
        typeof key === 'string' &&
        iaLogConfig.sensitiveFields.some((field) =>
  ...
  endOperation(
    ...
    if (status === 'failure') {
      logger.warn(
        LogCategory.IA,
        `${prefix} FIN (${statusText})${
          duration !== undefined ? ` - ${Math.round(duration)}ms` : ''
        }`,
        this.formatContext(enrichedContext),
      );
    } else {
  ...
  logPerformance(
    ...
    logger.info(
      LogCategory.IA,
      `${prefix} PERFORMANCE: ${Math.round(durationMs)}ms`,
      this.formatContext(context),
    );
  }
  ...
  logHttpRequest(
    ...
    if (iaLogConfig.redactSensitiveData) {
      safeUrl = url.replace(
        /([?&](api[_-]?key|token|secret|key|password)=)[^&]+/gi,
        '$1***REDACTED***',
      );
    }
