 ...
 */
import { IaProvider, IaProviderResponse, IaRequestOptions } from './iaProvider.interface';
...
export class GeminiProvider implements IaProvider {
  ...
  constructor(config: GeminiConfig) {
    ...
    this.apiBaseUrl = config.apiBaseUrl || 'https://generativelanguage.googleapis.com';
    ...
    logger.debug('Configuration Gemini initialisée', 'constructor', {
      ...
      apiKeyFormat: this.apiKey ? (this.apiKey.startsWith('AIza') ? 'Standard Google' : 'Non-standard') : 'Non définie'
    });
  ...
  async sendRequest(prompt: string, options?: IaRequestOptions): Promise<IaProviderResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        logger.error(`Réponse HTTP non-ok: ${response.status} ${response.statusText}`, 'sendRequest');
        ...
        try {
          ...
          logger.error('Impossible de parser la réponse d\'erreur en JSON', 'sendRequest');
          errorData = null;
        }
      ...
      logger.endOperation(operationId, 'sendRequest', 'success', { 
        model, latencyMs, textLength: text.length 
      });
      ...
      if (error instanceof Error && (error.name === 'AbortError' || error.name === 'TimeoutError' || error.message.includes('Timeout exceeded'))) {
        throw new IaError(
      ...
      if (error instanceof IaError) {
        ...
        logger.endOperation(operationId, 'sendRequest', 'failure', { errorType: error.type });
        throw error;
      }
      ...
      const iaError = new IaError(
        `Erreur lors de l'appel à Gemini: ${error instanceof Error ? error.message : String(error)}`,
        errorType,
        error
      );
  ...
  async checkAvailability(): Promise<boolean> {
    try {
      ...
      logger.warn(`Vérification de disponibilité de Gemini échouée: ${error instanceof Error ? error.message : String(error)}`, 'checkAvailability');
      return false;
    }
  }
