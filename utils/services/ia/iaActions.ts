 ...
 */
export async function processDatabaseAction(action: DetectedAction, userId: number): Promise<ProcessingResult> {
  try {
    ...
    if (!action.isValid) {
      ...
      return { 
        success: false, 
        error: `Action non valide: ${action.validationMessage || 'Raison inconnue'}`
      };
    }
    ...
    switch (action.type) {
      case 'ADD_MEAL':
        if (action.parsedData) {
          result = await processMealAction(action.parsedData as IaMealType, userId);
          return {
            success: true,
            data: {
      ...
      case 'ADD_PLAN':
        if (action.parsedData) {
          result = await processPlanAction(action.parsedData as IaPlanType, userId);
          return {
            success: true,
            data: {
      ...
      case 'ADD_INGREDIENT':
        if (action.parsedData) {
          result = await processIngredientAction(action.parsedData as IaIngredientType, userId);
          return {
            success: true,
            data: {
 ...
 */
async function processMealAction(mealData: IaMealType, userId: number): Promise<{ mealId?: number }> {
  try {
 ...
 * @returns Résultat du traitement
 */
async function processPlanAction(planData: IaPlanType, userId: number): Promise<{ planId?: number }> {
  try {
    ...
    logger.debug(LogCategory.IA, `Plan data to save: ${JSON.stringify(planToSave)}`);
    ...
    const queryClient = getQueryClient();
    if (queryClient) {
      logger.info(LogCategory.IA, `Invalidating plans cache for user ${userId}`);
      await queryClient.invalidateQueries({
        predicate: (query) => {
          const queryKey = query.queryKey[0];
 ...
 * @returns Résultat du traitement
 */
async function processIngredientAction(ingredientData: IaIngredientType, userId: number): Promise<{ ingredientId?: number, alreadyExists?: boolean }> {
  try {
 ...
 * @returns Résultat du traitement
 */
async function processNutritionPlanAction(planData: string, userId: number): Promise<{ processed: boolean }> {
  try {
 ...
 * @returns Résultat du traitement
 */
async function processMealRecommendationAction(recommendationData: string, userId: number): Promise<{ processed: boolean }> {
  try {
 ...
 * @returns Résultat du traitement
 */
async function processProgressAnalysisAction(analysisData: string, userId: number): Promise<{ processed: boolean }> {
  try {
 ...
 * @returns Résultat du traitement
 */
async function processNutritionAdviceAction(adviceData: string, userId: number): Promise<{ processed: boolean }> {
  try {
