...
export const DEFAULT_RETRY_OPTIONS: RetryOptions = {
  ...
  onRetry: (error, attempt, delayMs) => {
    if (error instanceof IaError) {
      ...
      logger.info(
        LogCategory.IA, 
        `retryWithBackoff: Tentative ${attempt} après erreur: ${error instanceof Error ? error.message : String(error)}. Attente: ${delayMs}ms`
      );
    }
  }
};
 ...
 */
export const isRetryableError = (error: any, options: RetryOptions): boolean => {
 ...
 */
export const calculateBackoffDelay = (attempt: number, options: RetryOptions): number => {
  ...
  for (let attempt = 0; attempt <= fullOptions.maxRetries; attempt++) {
    try {
      ...
      if (attempt >= fullOptions.maxRetries) {
        logger.error(
          LogCategory.IA, 
          `retryWithBackoff: Échec après ${attempt + 1} tentatives. Dernière erreur: ${error instanceof Error ? error.message : String(error)}`
        );
        break;
      }
      ...
      if (!isRetryableError(error, fullOptions)) {
        logger.warn(
          LogCategory.IA, 
          `retryWithBackoff: Erreur non récupérable, arrêt des tentatives: ${error instanceof Error ? error.message : String(error)}`
        );
        break;
      }
...
export function withRetry<T extends (...args: any[]) => Promise<any>>(
  ...
  return async (...args: Parameters<T>): Promise<ReturnType<T> extends Promise<infer R> ? R : ReturnType<T>> => {
    return retryWithBackoff(() => fn(...args), options) as ReturnType<T> extends Promise<infer R> ? R : ReturnType<T>;
  };
}
