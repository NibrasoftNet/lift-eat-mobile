...
export class FallbackHandler {
   ...
   */
  static generateIngredientFallback(
    context: FallbackContext,
  ): IaIngredientType {
    logger.info(
      "Génération d'un ingrédient de fallback",
  ...
  static generateMealFallback(context: FallbackContext): IaMealType {
    logger.info("Génération d'un repas de fallback", 'generateMealFallback', {
      context,
    });
    ...
    let description =
      "Repas généré automatiquement en l'absence de réponse valide de l'IA.";

    switch (mealType) {
      case MealTypeEnum.BREAKFAST:
        name = 'Petit-déjeuner équilibré';
        description =
          'Un petit-déjeuner complet avec un bon équilibre de macronutriments.';
        break;
      case MealTypeEnum.LUNCH:
        name = 'Déjeuner équilibré';
        description =
          'Un déjeuner complet avec un bon équilibre de macronutriments.';
        break;
      case MealTypeEnum.DINNER:
        name = 'Dîner équilibré';
        description =
          'Un dîner léger mais nutritif avec un bon équilibre de macronutriments.';
        break;
    ...
    const fallbackMeal: IaMealType = {
      ...
      description,
      instructions:
        'Combinez tous les ingrédients et préparez selon vos préférences.',
    ...
    logger.debug('Repas de fallback généré', 'generateMealFallback', {
      meal: fallbackMeal,
    });
  ...
  static generatePlanFallback(context: FallbackContext): IaPlanType {
    logger.info(
      "Génération d'un plan nutritionnel de fallback",
    ...
    const fallbackPlan: IaPlanType = {
      ...
      meals: [
        // Petit déjeuner
        {
          ...
          description:
            'Un petit-déjeuner nutritif pour bien commencer la journée.',
    ...
    logger.debug(
      'Plan nutritionnel de fallback généré',
      'generatePlanFallback',
      { plan: fallbackPlan },
    );
  ...
  static generateTextResponseFallback(context: FallbackContext): string {
    logger.info(
      "Génération d'une réponse textuelle de fallback",
      'generateTextResponseFallback',
      { context },
    );

    let fallbackResponse =
      "Je n'ai pas pu générer une réponse complète. Voici quelques informations générales qui pourraient vous être utiles.";
    ...
    if (context.originalQuery) {
      ...
      if (query.includes('repas') || query.includes('meal')) {
        fallbackResponse =
          "Je n'ai pas pu générer un repas complet. Pour créer un repas équilibré, essayez d'inclure une protéine, des glucides complexes et des légumes. Par exemple, du poulet grillé avec du riz complet et des légumes sautés.";
      } else if (
        query.includes('plan') ||
        query.includes('régime') ||
        query.includes('diet')
      ) {
        fallbackResponse =
          "Je n'ai pas pu générer un plan nutritionnel complet. Un bon plan nutritionnel devrait inclure 3-4 repas équilibrés par jour, avec un bon équilibre de protéines, glucides et lipides.";
      } else if (query.includes('ingrédient') || query.includes('ingredient')) {
        fallbackResponse =
          "Je n'ai pas pu trouver les informations complètes sur cet ingrédient. Essayez de consulter une base de données nutritionnelle comme OpenFoodFacts pour des informations détaillées.";
      }
    }
    ...
    logger.debug(
      'Réponse textuelle de fallback générée',
      'generateTextResponseFallback',
    ...
    try {
      ...
      switch (type) {
        case FallbackType.INGREDIENT:
          ...
          logger.endOperation('getFallback', 'getFallback', 'success', {
            type,
        ...
        default:
          ...
          logger.endOperation('getFallback', 'getFallback', 'warning', {
            type,
      ...
      logger.error(
        `Erreur lors de la génération du fallback: ${
          error instanceof Error ? error.message : String(error)
        }`,
        'getFallback',
        error,
      );
      logger.endOperation('getFallback', 'getFallback', 'failure', {
        type,
        fallbackProvided: false,
      });
