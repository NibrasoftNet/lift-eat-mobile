...
class MealDrawerUIService implements MealDrawerServiceInterface {
  ...
  async addMealsToPlan(dailyPlanId: number, selectedMeals: MealWithQuantity[]): Promise<{ success: boolean; errors?: string[] }> {
    try {
      logger.info(LogCategory.UI, 'Adding meals to plan', { dailyPlanId, mealsCount: selectedMeals.length });
      ...
      for (const meal of selectedMeals) {
        try {
          ...
          if (!result.success) {
            errors.push(`Failed to add meal ${meal.id}: ${result.error || 'Unknown error'}`);
          }
        } catch (error) {
