...
class NutritionUIService {
  ...
  formatNutritionFor100g(
    ...
    try {
      ...
      const engineResult = nutritionEngine.normalizeForDisplay(
        macros,
        totalWeight,
        displayMode,
      );
      ...
      logger.error(
        LogCategory.UI,
        'Erreur lors du formatage des valeurs nutritionnelles pour affichage à 100g',
        {
      ...
      return {
        ...
        cssClasses: {
          container: 'nutrition-error',
          label: 'nutrition-label-error',
        },
      };
    }
  }
  ...
  formatAdjustmentFactor(
    ...
    try {
      ...
      return includePrefix
        ? `Ajustement: ${factorWithSymbol}`
        : factorWithSymbol;
    } catch (error) {
      logger.warn(
        LogCategory.UI,
        "Erreur lors du formatage du facteur d'ajustement",
        {
          error: error instanceof Error ? error.message : String(error),
          factor,
        },
  ...
  formatNutrientValue(
    ...
    try {
      ...
      logger.warn(
        LogCategory.UI,
        "Erreur lors du formatage d'une valeur nutritionnelle",
        {
          error: error instanceof Error ? error.message : String(error),
          value,
          type,
      ...
      return (
        value.toString() +
        (includeUnit ? (type === 'calories' ? ' kcal' : ' g') : '')
      );
    }
  }
  ...
  createNutritionDisplayText(
    ...
    try {
      const result = this.formatNutritionFor100g(
        macros,
        totalWeight,
        displayMode,
      );
      ...
      const caloriesText = this.formatNutrientValue(
        result.normalizedMacros.calories,
        'calories',
      );
      const proteinText = this.formatNutrientValue(
        result.normalizedMacros.protein,
        'protein',
      );
      const carbsText = this.formatNutrientValue(
        result.normalizedMacros.carbs,
        'carbs',
      );
      const fatText = this.formatNutrientValue(
        result.normalizedMacros.fat,
        'fat',
      );
      ...
      logger.error(
        LogCategory.UI,
        "Erreur lors de la création du texte d'affichage nutritionnel",
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
   ...
   */
  getNutritionCssClasses(
    isStandardized: boolean = true,
    isEmpty: boolean = false,
  ): Record<string, string> {
    return {
      container: `nutrition-container ${isStandardized ? 'standardized' : ''} ${
        isEmpty ? 'empty' : ''
      }`,
      label: isStandardized
        ? 'nutrition-label nutrition-label-100g'
        : 'nutrition-label',
