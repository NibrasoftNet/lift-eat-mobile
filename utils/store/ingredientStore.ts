...
const calculateIngredientsTotalMacros = (ingredients: IngredientWithStandardProps[]) => {
  try {
    ...
    const cacheKey = ingredients.map(ing => 
    ...
    conditionalLog('Calcul des macros pour ingrédients:', 
      ingredients.map(ing => ({
    ...
    const totals = ingredients.reduce(
      (acc, ingredient) => {
        ...
        const calories = isNaN(ingredient.calories) ? 0 : ingredient.calories || 0;
    ...
    const finalCalories = Math.abs(calculatedCalories - totals.calories) > 50 ? 
...
const getIngredientsHash = (ingredients: IngredientWithStandardProps[]): string => {
  return ingredients.map(ing => `${ing.ingredientStandardId}-${ing.quantity}`).join('|');
};
...
const recalculateTotalMacros = (
  ...
  conditionalLog(`[STORE] Poids d'affichage demandé: ${mealWeight || 'non spécifié'}g`);
  ...
  conditionalLog(`[STORE] Poids total des ingrédients: ${totalIngredientsWeight}g`);
  ...
  const adjustedMacros = adjustMacrosByFinalWeight(rawMacros, totalIngredientsWeight, mealWeight);
...
export const useIngredientStore = create<IngredientStore>((set) => ({
    ...
    set((state) => {
      try {
        ...
        if (!isValidWeight(weight)) {
          console.log('[STORE] Poids spécifié trop faible, utilisation de valeurs par défaut');
          return {
            mealWeight: weight,
            totalMacros: {
              totalCalories: 0,
  ...
  set(produce((state: IngredientStore) => {
    try {
      const existingIndex = state.selectedIngredients.findIndex(
        (ing) => ing.ingredientStandardId === ingredient.id
      );
      ...
      if (existingIndex !== -1) {
        ...
        state.selectedIngredients.push(mapToIngredientWithStandard(ingredient));
      }
      ...
      state.totalMacros = recalculateTotalMacros(state.selectedIngredients, state.mealWeight);
        ...
        const updatedIngredients = state.selectedIngredients.map((ing) => {
          if (ing.ingredientStandardId === ingredientStandardId) {
            ...
            if (!ing.ingredientsStandard) {
              console.error('ingredientsStandard is undefined for ingredient:', ing);
            ...
            const standardMacros = ing.ingredientsStandard.carbs + ing.ingredientsStandard.protein + ing.ingredientsStandard.fat;
            // Si les valeurs nutritionnelles sont incohérentes avec la quantité, elles sont probablement pour 100g
            const valuesPerHundredGrams = standardMacros > ing.ingredientsStandard.quantity * 1.3;
            ...
            if (valuesPerHundredGrams) {
              ...
              updatedCalories = (ing.ingredientsStandard.calories * newQuantity) / 100;
              updatedCarbs = (ing.ingredientsStandard.carbs * newQuantity) / 100;
              updatedFat = (ing.ingredientsStandard.fat * newQuantity) / 100;
              updatedProtein = (ing.ingredientsStandard.protein * newQuantity) / 100;
            } else {
