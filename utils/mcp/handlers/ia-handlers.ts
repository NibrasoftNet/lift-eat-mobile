...
export async function handleGetUserContext(
  ...
  try {
    logger.info(
      ...
      'Generating user context via MCP Handler',
    ...
    if (!user) {
      logger.warn(
        LogCategory.DATABASE,
        'User not found for context generation',
        { userId: params.userId },
      );
    ...
    context += `- Weight: ${user.weight || 'Unknown'} ${
      user.weightUnit || ''
    }\n`;
    context += `- Height: ${user.height || 'Unknown'} ${
      user.heightUnit || ''
    }\n`;
    context += `- Physical Activity Level: ${
      user.physicalActivity || 'Unknown'
    }\n`;
    ...
    if (nutritionGoals) {
      ...
      context += `- Target Weight: ${
        nutritionGoals.targetWeight
          ? `${nutritionGoals.targetWeight} ${user.weightUnit || ''}`
          : 'Not set'
      }\n`;
      context += `- Daily Calories: ${
        nutritionGoals.dailyCalories || 'Not set'
      }\n`;
      context += `- Macronutrient Ratios: ${
        nutritionGoals.proteinPercentage || 'Not set'
      }% protein, ${nutritionGoals.carbsPercentage || 'Not set'}% carbs, ${
        nutritionGoals.fatPercentage || 'Not set'
      }% fat\n`;
    } else {
    ...
    logger.info(
      LogCategory.DATABASE,
      'Enhanced user context generated successfully',
      { userId: params.userId },
    );
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error generating user context',
    };
  } finally {
...
export async function handleGetUserPreferences(
  ...
  try {
    logger.info(
      LogCategory.DATABASE,
      'Fetching user preferences via MCP Handler',
      { userId: params.userId },
    );
    ...
    if (!user) {
      logger.warn(LogCategory.DATABASE, 'No preferences found for user', {
        userId: params.userId,
      });
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error fetching user preferences',
    };
  } finally {
...
export async function handleGetUserFavoriteMeals(
  ...
  try {
    logger.info(LogCategory.DATABASE, 'Fetching user meals via MCP Handler', {
      userId: params.userId,
    });
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : 'Unknown error fetching meals',
    };
  } finally {
...
export async function handleGetUserActivePlans(
  ...
  try {
    logger.info(
      LogCategory.DATABASE,
      'Fetching user active plans via MCP Handler',
      { userId: params.userId },
    );
    ...
    if (userPlans.length === 0) {
      logger.info(LogCategory.DATABASE, 'No plans found for user', {
        userId: params.userId,
      });
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error fetching active plans',
    };
  } finally {
...
export async function handleSaveNutritionAdvice(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      'Saving nutrition advice via MCP Handler',
      {
        userId: params.userId,
        type: params.type,
      },
    ...
    if (
      ...
      !Object.values(NutritionAdviceType).includes(
        params.type as NutritionAdviceType,
      )
    ) {
      ...
      logger.warn(
        LogCategory.DATABASE,
        `Invalid advice type: ${params.type}, defaulting to GENERAL`,
      );
    }

    // Insérer le conseil dans la base de données
    const result = await db
      .insert(nutritionAdvice)
      .values({
        title: params.title,
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully saved nutrition advice for user ${params.userId}`,
    );
    return {
      success: true,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error saving nutrition advice',
    };
  } finally {
...
export async function handleGetNutritionAdvice(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      'Fetching nutrition advice via MCP Handler',
      {
        userId: params.userId,
    ...
    const whereConditions = [eq(nutritionAdvice.userId, params.userId)];
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully retrieved ${adviceList.length} nutrition advice items for user ${params.userId}`,
    );

    // Convertir les résultats en NutritionAdviceProps
    const typedAdviceList: NutritionAdviceProps[] = adviceList.map(
      (advice) => ({
        id: advice.id,
        title: advice.title,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error fetching nutrition advice',
    };
  } finally {
...
export async function handleUpdateAdviceFeedback(
  ...
  try {
    logger.info(
      LogCategory.DATABASE,
      'Updating nutrition advice feedback via MCP Handler',
      {
    ...
    if (!advice) {
      logger.warn(
        LogCategory.DATABASE,
        'Advice not found for feedback update',
        { adviceId: params.adviceId },
      );
    ...
    logger.info(
      LogCategory.DATABASE,
      'Nutrition advice feedback updated successfully',
      { adviceId: params.adviceId },
    );
    return { success: true };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      'Error updating nutrition advice feedback',
      {
        adviceId: params.adviceId,
        error: error instanceof Error ? error.message : String(error),
      },
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error updating advice feedback',
    };
  } finally {
...
export async function handleGetUserActivityHistory(
  ...
  try {
    logger.info(
      LogCategory.DATABASE,
      'Fetching user activity history via MCP Handler',
      {
        userId: params.userId,
    ...
    const userCurrentPlanQuery = await db
      .select()
      .from(plan)
      .where(and(eq(plan.userId, params.userId), eq(plan.current, true)))
      .limit(1);
    ...
    const mealProgressRecords = await db
      ...
      .innerJoin(
        dailyProgress,
        eq(dailyMealProgress.dailyProgressId, dailyProgress.id),
      )
      .where(
        and(
          eq(dailyProgress.userId, params.userId),
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error fetching activity history',
    };
  } finally {
