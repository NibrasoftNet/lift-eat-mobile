 ...
 */
export async function handleGetMealsList(
  db: any,
  ...
  try {
    ...
    logger.info(
      ...
      `Getting meals list for user ${userId} with filter ${filter} via MCP Server`,
    );
    ...
    logger.info(
      LogCategory.DATABASE,
      `Retrieved ${mealsResult.length} meals for user ${userId} via MCP Server`,
    );
    return { success: true, meals: mealsResult };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      `Error in handleGetMealsList: ${
 ...
 */
export async function handleGetMealDetails(
  db: any,
  params: GetMealDetailsParams & { userId?: number },
): Promise<GetMealDetailsResult> {
  const { mealId } = params;
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Getting meal details for meal ${mealId}${
    ...
    logger.info(
      LogCategory.DATABASE,
      `Retrieved meal details and ${mealIngredientsWithDetails.length} ingredients for meal ${mealId} via MCP Server`,
    );
    return {
      success: true,
      meal: mealDetails[0],
...
export async function handleCreateMeal(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Creating meal for user ${userId} via MCP Server`,
    );
    ...
    if (userExists.length === 0) {
      logger.warn(
        LogCategory.DATABASE,
        `Cannot create meal: User with ID ${userId} not found`,
      );
    ...
    return await db.transaction(async (tx: any) => {
      ...
      if (ingredients && ingredients.length > 0) {
        ...
        logger.info(
          ...
          `Calculated total weight for meal: ${totalWeight}g from ${ingredients.length} ingredients`,
        );
      }
      ...
      if (totalWeight < MIN_WEIGHT) {
        logger.warn(
          LogCategory.NUTRITION,
          `Invalid total weight (${totalWeight}g), using minimum value of ${MIN_WEIGHT}g`,
        );
      ...
      if (
        ...
        if (Math.abs(totalWeight - STANDARD_WEIGHT) > 0.01) {
          logger.info(
            LogCategory.NUTRITION,
            `Normalizing meal macros from ${totalWeight}g to ${STANDARD_WEIGHT}g`,
          );
      ...
      if (
        ...
        Math.abs(totalWeight - STANDARD_WEIGHT) > 0.01
      ) {
        logger.info(
          LogCategory.NUTRITION,
          `Normalizing ingredients from ${totalWeight}g to ${STANDARD_WEIGHT}g`,
        );
        // Utiliser la fonction de normalisation que nous avons créée
        const result = nutritionCoreService.normalizeIngredientsTo100g(
          ingredients,
          totalWeight,
        );
        // S'assurer que normalizedIngredients n'est jamais undefined
        normalizedIngredients =
          result.normalizedIngredients || ingredients || [];

        logger.info(
          LogCategory.NUTRITION,
          `Applied normalization factor of ${result.normalizationFactor.toFixed(
            4,
          )}x to ingredients. ` +
      ...
      logger.info(
        LogCategory.DATABASE,
        `Successfully created meal ${mealId} for user ${userId} with total weight ${totalWeight}g via MCP Server`,
      );
      ...
      if (ingredients && ingredients.length > 0) {
        logger.info(
          LogCategory.DATABASE,
          `Verifying and adding ${ingredients.length} ingredients to meal ${mealId}`,
        );
        ...
        missingIngredients = ingredients.filter(
          (ingredient) => !existingIngredientIds.includes(ingredient.id),
        );

        if (missingIngredientIds.length > 0) {
          logger.warn(
            LogCategory.DATABASE,
            `Some ingredients don't exist in the database: ${missingIngredientIds.join(
              ', ',
            )}`,
          );
          ...
          if (validIngredients.length === 0) {
            logger.error(
              LogCategory.DATABASE,
              `No valid ingredients found for meal ${mealId}`,
            );
          ...
          logger.info(
            LogCategory.DATABASE,
            `Proceeding with ${validIngredients.length} valid ingredients out of ${ingredients.length}`,
          );
          ...
          const mealIngredientsData = validIngredients.map((ingredient) => {
            ...
            const normalizedVersion = normalizedIngredients.find(
              (ni) => ni.id === ingredient.id,
            );
            ...
            return {
              ...
              quantity:
                normalizedVersion?.quantity !== undefined
                  ? normalizedVersion.quantity
                  : ingredient.quantity || 0,
              // Utiliser les valeurs nutritionnelles normalisées si disponibles
              calories:
                normalizedVersion?.calories !== undefined
                  ? normalizedVersion.calories
                  : ingredient.calories || 0,
              carbs:
                normalizedVersion?.carbs !== undefined
                  ? normalizedVersion.carbs
                  : ingredient.carbs || 0,
              fat:
                normalizedVersion?.fat !== undefined
                  ? normalizedVersion.fat
                  : ingredient.fat || 0,
              protein:
                normalizedVersion?.protein !== undefined
                  ? normalizedVersion.protein
          ...
          logger.info(
            LogCategory.DATABASE,
            `Added ${validIngredients.length} valid ingredients to meal ${mealId}`,
          );
        } else {
          // Tous les ingrédients existent, continuer normalement
          // Utiliser les ingrédients normalisés directement
          const mealIngredientsData = normalizedIngredients.map(
            (ingredient) => ({
              mealId: mealId,
              ingredientStandardId: ingredient.id,
          ...
          logger.info(
            LogCategory.DATABASE,
            `Added ${ingredients.length} ingredients to meal ${mealId}`,
          );
        }
      } else {
        logger.info(
          LogCategory.DATABASE,
          `No ingredients provided for meal ${mealId}`,
        );
      }
...
export async function handleAddMealToDailyPlan(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Adding meal ${mealId} to daily plan ${dailyPlanId} (quantity: ${quantity}) via MCP Server`,
    );
    ...
    if (!meal) {
      logger.warn(
        LogCategory.DATABASE,
        `Cannot add meal: Meal with ID ${mealId} not found`,
      );
      return { success: false, error: `Meal with ID ${mealId} not found` };
    }
    ...
    if (!currentDailyPlan) {
      logger.warn(
        LogCategory.DATABASE,
        `Cannot add meal: Daily plan with ID ${dailyPlanId} not found`,
      );
      return {
        success: false,
 ...
 * @returns Résultat de l'opération avec l'ID du repas créé
 */
export async function handleCreateNewMeal(
  db: any,
  params: CreateNewMealParams,
  ...
  try {
    ...
    return await db.transaction(async (tx: typeof db) => {
      logger.info(
        LogCategory.DATABASE,
        `Creating new meal "${data.name}" via MCP Server`,
      );
      ...
      if (selectedIngredients && selectedIngredients.length > 0) {
        ...
        logger.info(
          LogCategory.NUTRITION,
          `Calculated total weight for new meal: ${totalWeight}g from ${selectedIngredients.length} ingredients`,
        );
      }
      ...
      if (selectedIngredients && selectedIngredients.length > 0) {
        logger.info(
          LogCategory.DATABASE,
          `Adding ${selectedIngredients.length} ingredients to meal ${mealId}`,
        );
        ...
        if (Math.abs(totalWeight - STANDARD_WEIGHT) > 0.01) {
          ...
          normalizedIngredients =
            result.normalizedIngredients || selectedIngredients;
        ...
        logger.info(
          LogCategory.DATABASE,
          `Added ${selectedIngredients.length} ingredients to meal ${mealId}`,
        );
      }
...
export async function handleUpdateMeal(
  ...
  try {
    ...
    if (!userId) {
      logger.error(
        LogCategory.DATABASE,
        `Attempted to update meal ${mealId} without providing userId`,
      );
      return {
        success: false,
        error: 'User ID is required for updating meals',
      };
    }

    logger.info(
      LogCategory.DATABASE,
      `Updating meal ${mealId} for user ${userId} via MCP Server`,
    );
    ...
    if (!mealExists.length) {
      logger.warn(
        LogCategory.DATABASE,
        `Meal with ID ${mealId} not found or does not belong to user ${userId}`,
      );
      return {
        success: false,
    ...
    await db.transaction(async (tx: any) => {
      ...
      if (ingredients && ingredients.length > 0) {
        ...
        logger.info(
          LogCategory.NUTRITION,
          `Total weight before update normalisation: ${totalWeight}g`,
        );

        // Appliquer normalisation à 100g si nécessaire
        if (Math.abs(totalWeight - STANDARD_WEIGHT) > 0.01) {
          const { normalizedIngredients: nIngs, normalizationFactor } =
            nutritionCoreService.normalizeIngredientsTo100g(
              ingredients,
              totalWeight,
            );
          ...
          logger.info(
            LogCategory.NUTRITION,
            `Applied normalization factor of ${normalizationFactor.toFixed(
              4,
            )}x to ingredients for update.`,
          );
        }
        ...
        normalizedMacros.totalCalories = Math.round(
          normalizedMacros.totalCalories,
        );
        ...
        normalizedMacros.totalProtein =
          +normalizedMacros.totalProtein.toFixed(2);
      }
      ...
      if (hasImageUri) {
        ...
        logger.info(
          LogCategory.DATABASE,
          `Converted imageUri to image field for meal ${mealId}`,
        );
      }

      // Mettre à jour le repas
      await tx.update(meals).set(updateData).where(eq(meals.id, mealId));
      ...
      if (normalizedIngredients && normalizedIngredients.length > 0) {
        ...
        logger.info(
          LogCategory.DATABASE,
          `Deleted existing ingredients for meal ${mealId}`,
        );
        ...
        logger.info(
          LogCategory.DATABASE,
          `Added ${normalizedIngredients.length} new ingredients to meal ${mealId}`,
        );
      }
    });
...
export async function handleDeleteMeal(
  ...
  try {
    ...
    if (!userId) {
      logger.error(
        LogCategory.DATABASE,
        `Attempted to delete meal ${mealId} without providing userId`,
      );
      return {
        success: false,
        error: 'User ID is required for deleting meals',
      };
    }

    logger.info(
      LogCategory.DATABASE,
      `Deleting meal ${mealId} for user ${userId} via MCP Server`,
    );
    ...
    if (!mealExists) {
      logger.warn(
        LogCategory.DATABASE,
        `Meal ${mealId} not found for deletion`,
      );
      return {
        success: false,
    ...
    await db.transaction(async (tx: any) => {
      ...
      logger.info(
        LogCategory.DATABASE,
        `Deleted ingredients for meal ${mealId}`,
      );
      ...
      if (mealInPlans.length > 0) {
        ...
        logger.info(
          LogCategory.DATABASE,
          `Deleted ${mealInPlans.length} plan relations for meal ${mealId}`,
        );
      }

      // Supprimer le repas
      await tx.delete(meals).where(eq(meals.id, mealId));
