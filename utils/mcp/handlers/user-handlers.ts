...
export async function handleUpdateUserPreferences(
  ...
  try {
    ...
    logger.info(
      ...
      `Updating preferences for user ${userId} via MCP Server`,
    );
    ...
    if (userExists.length === 0) {
      logger.warn(
        LogCategory.DATABASE,
        `Cannot update preferences: User with ID ${userId} not found`,
      );
    ...
    if (
      ...
      if (preferences.gender === 'MALE') {
        bmr =
          88.362 +
          13.397 * preferences.weight +
          4.799 * preferences.height -
          5.677 * preferences.age;
      } else {
        bmr =
          447.593 +
          9.247 * preferences.weight +
          3.098 * preferences.height -
          4.33 * preferences.age;
      }
      ...
      const activityMultiplier =
        activityMultipliers[preferences.physicalActivity] || 1.55; // Valeur par défaut si non trouvée
      const totalCalories = Math.round(bmr * activityMultiplier);
      console.log(
        `Calculated calories for user ${userId}: ${totalCalories} (not saved as no field exists)`,
      );
    }
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully updated preferences for user ${userId} via MCP Server`,
    );
    return { success: true };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      `Error in handleUpdateUserPreferences: ${
...
export async function handleUpdateUserNutritionPreferences(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Updating nutrition preferences for user ${userId} via MCP Handler`,
    );
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully updated nutrition preferences for user ${userId} via MCP Handler`,
    );
    return { success: true };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      `Error in handleUpdateUserNutritionPreferences: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return {
...
export async function handleGetUserDetails(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Getting user details for user ${userId} (requested by ${
    ...
    if (requestingUserId && userId !== requestingUserId) {
      logger.warn(
        LogCategory.DATABASE,
        `Unauthorized attempt to access user ${userId} details by user ${requestingUserId}`,
      );
      return {
        success: false,
        error: "Not authorized to access this user's details",
      };
    }
    ...
    logger.info(
      LogCategory.DATABASE,
      `Retrieved details for user ${userId} via MCP Server`,
    );
    return { success: true, user: userDetails[0] };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      `Error in handleGetUserDetails: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return {
...
export async function handleCreateUser(
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully created user with ID ${userId} via MCP Server`,
    );
    return { success: true, userId };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      `Error in handleCreateUser: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return {
 ...
 */
export async function handleValidateUserExists(
  db: any,
  params: ValidateUserExistsParams,
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Validating existence of user ${userId} via MCP Server`,
    );
    ...
    logger.info(
      LogCategory.DATABASE,
      `User ${userId} ${exists ? 'exists' : 'does not exist'} in database`,
    );
    return {
      success: true,
      exists,
    ...
    logger.error(
      LogCategory.DATABASE,
      `Error in handleValidateUserExists: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return {
 ...
 */
export async function handleGetDefaultUser(
  db: any,
  params: GetDefaultUserParams,
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      'Getting default user via MCP Server',
      params,
    );
    ...
    if (params.userId) {
      logger.info(
        LogCategory.DATABASE,
        `Trying to get user with ID: ${params.userId}`,
      );
      ...
      if (userResult.length > 0) {
        logger.info(
          LogCategory.DATABASE,
          `Found user with ID: ${params.userId}`,
        );
        return {
          success: true,
          user: userResult[0],
      ...
      logger.warn(
        LogCategory.DATABASE,
        `User with ID ${params.userId} not found, trying to get default user`,
      );
    }
    ...
    if (defaultUserResult.length > 0) {
      logger.info(
        LogCategory.DATABASE,
        `Found default user with ID: ${defaultUserResult[0].id}`,
      );
      return {
        success: true,
 ...
 */
export async function handleGenerateUserContext(
  db: any,
  params: GenerateUserContextParams,
): Promise<GenerateUserContextResult> {
  const { userId } = params;
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully generated context for user ${userId} via MCP Server`,
    );
    return { success: true, context: formattedContext };
  } catch (error) {
    logger.error(
      LogCategory.DATABASE,
      `Error in handleGenerateUserContext: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return {
