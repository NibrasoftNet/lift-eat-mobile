...
export async function handleCalculateNormalizedNutrition(
  ...
  try {
    ...
    logger.info(
      ...
      `Calculating normalized nutrition for meal ${mealId}`,
    );

    // 1. Récupérer les détails du repas
    let query = db.select().from(meals).where(eq(meals.id, mealId));
    ...
    if (!ingredientsWithDetails || ingredientsWithDetails.length === 0) {
      logger.warn(
        LogCategory.NUTRITION,
        `No ingredients found for meal ${mealId}`,
      );
      return {
        success: false,
    ...
    if (totalWeight <= 0) {
      logger.info(
        LogCategory.NUTRITION,
        `Meal weight not found in database (${totalWeight}g), calculating from ingredients`,
      );
    ...
    logger.debug(
      LogCategory.NUTRITION,
      `Using meal weight: ${totalWeight}g for meal ${mealId}`,
    );

    if (totalWeight <= 0) {
      logger.warn(
        LogCategory.NUTRITION,
        `Invalid total weight (${totalWeight}g) for meal ${mealId}`,
      );
      return {
        success: false,
    ...
    logger.info(
      LogCategory.NUTRITION,
      `Utilisation du mode d'affichage: ${displayMode}, facteur: ${adjustmentFactor.toFixed(
        2,
      )}, texte: ${displayText}`,
    );
    ...
    for (const item of ingredientsWithDetails) {
      ...
      if (ingredient.calories)
        totalCalories += (ingredient.calories * ingredientQuantity) / 100;
      if (ingredient.protein)
        totalProtein += (ingredient.protein * ingredientQuantity) / 100;
      if (ingredient.carbs)
        totalCarbs += (ingredient.carbs * ingredientQuantity) / 100;
      if (ingredient.fat)
        totalFats += (ingredient.fat * ingredientQuantity) / 100;
    }
    ...
    logger.error(
      LogCategory.NUTRITION,
      `Error in handleCalculateNormalizedNutrition: ${
...
export async function handleGetMealWeight(
  ...
  try {
    ...
    logger.info(
      LogCategory.NUTRITION,
      `Getting meal weight for meal ${mealId}`,
    );
    ...
    if (userId) {
      ...
      if (!mealExists) {
        logger.warn(
          LogCategory.NUTRITION,
          `Meal with ID ${mealId} not found or access denied`,
        );
        return {
          success: false,
    ...
    logger.info(
      LogCategory.NUTRITION,
      `Total weight for meal ${mealId}: ${totalWeight}g from ${ingredients.length} ingredients`,
    );
...
export async function handleGetDailyPlanNutrition(
  ...
  try {
    ...
    logger.info(
      LogCategory.NUTRITION,
      `Calculating daily plan nutrition for plan ${planId} (user: ${userId})`,
    );
    ...
    if (
      !planDetails ||
      !planDetails.parentPlan ||
      planDetails.parentPlan.userId !== userId
    ) {
      logger.warn(
        LogCategory.NUTRITION,
        `Daily plan with ID ${planId} not found or access denied for user ${userId}`,
      );
      return {
        success: false,
        error: `Daily plan with ID ${planId} not found or access denied`,
      };
    }
    ...
    if (!planMeals || planMeals.length === 0) {
      logger.warn(
        LogCategory.NUTRITION,
        `No meals found for daily plan ${planId}`,
      );
      return {
        success: true,
        nutrition: {
...
export async function handleGetMacroBreakdown(
  ...
  try {
    ...
    const shouldNormalize =
      displayMode && displayMode !== NutritionDisplayMode.AS_IS;

    logger.info(
      LogCategory.NUTRITION,
      `Getting macro breakdown for meal ${mealId}, mode=${
        displayMode || 'AS_IS'
      }`,
    );
      ...
      if (!normalizedResult.success || !normalizedResult.normalizedNutrition) {
        return {
          ...
          error:
            ...
            'Failed to calculate normalized nutrition',
        };
      }
    ...
    logger.error(
      LogCategory.NUTRITION,
      `Error in handleGetMacroBreakdown: ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
    return {
