...
export async function handleCreatePlan(db: any, params: CreatePlanParams): Promise<PlanResult> {
  try {
    ...
    const physicalActivity = anyData.physicalActivity || user.physicalActivity || PhysicalActivityEnum.MODERATE;
    ...
    try {
      ...
      logger.info(LogCategory.NUTRITION, `Calculated ${calculatedCalories} daily calories for user ${userId}`, {
        weight, height, age, gender, physicalActivity, bmr, multiplier
      });
    } catch (calcError) {
      logger.error(LogCategory.NUTRITION, `Error calculating calories: ${calcError instanceof Error ? calcError.message : String(calcError)}`);
    ...
    logger.debug(LogCategory.NUTRITION, `Calculated macronutrients for user ${userId}`, {
...
export async function handleCalculateMealNutrition(
  ...
  try {
    ...
    logger.debug(
      ...
      `Calcul nutritionnel: q=${quantity}g, facteur=${quantityRatio.toFixed(2)}, ` +
      `Cal=${mealInfo.calories} -> ${Math.round(mealInfo.calories * quantityRatio)}`
    );
...
export async function handleGetDailyPlanNutrition(
  ...
  try {
    ...
    logger.info(LogCategory.DATABASE, `Calculating nutrition for daily plan ${dailyPlanId} for user ${userId}`);
    ...
    if (dailyPlanInfo.length === 0) {
      logger.warn(LogCategory.DATABASE, `Daily plan with ID ${dailyPlanId} not found or user not authorized`);
      return { success: false, error: `Daily plan with ID ${dailyPlanId} not found or user not authorized` };
    }
    ...
    logger.error(LogCategory.DATABASE, `Error in handleGetDailyPlanNutrition: ${error instanceof Error ? error.message : String(error)}`);
    return { 
      success: false, 
      error: error instanceof Error ? error.message : String(error) 
    };
