 ...
 */
export async function handleRemoveMealFromDailyPlan(
  db: any,
  params: { dailyPlanId: number; mealId: number },
) {
  ...
  try {
    ...
    logger.info(
      LogCategory.DATABASE,
      `Successfully removed meal ${mealId} from daily plan ${dailyPlanId}`,
    );
    ...
    logger.error(
      LogCategory.DATABASE,
      `Error in handleRemoveMealFromDailyPlan: ${
