...
export function normalizeMacrosToReferenceWeight(
  ...
  try {
    ...
    const normalized: NormalizedNutrients = {
      calories: standardizeNutritionalRounding(macros.calories * normalizationFactor, 'calories'),
      protein: standardizeNutritionalRounding(macros.protein * normalizationFactor, 'protein'),
      carbs: standardizeNutritionalRounding(macros.carbs * normalizationFactor, 'carbs'),
      fat: standardizeNutritionalRounding(macros.fat * normalizationFactor, 'fat'),
      unit: macros.unit || 'g',
      referenceWeight,
      originalWeight,
      normalizationFactor,
...
export function validateMacroBalance(
  ...
  const diff = parseFloat(Math.abs(calculatedCalories - macros.calories).toFixed(3));
  
  // Tolérance en valeur absolue (pourcentage des calories calculées)
  const toleranceValue = parseFloat(((calculatedCalories * tolerancePercent) / 100).toFixed(3));
 ...
 */
export function calculateCaloriesFromMacros(macros: MacroNutrientsBase): number {
