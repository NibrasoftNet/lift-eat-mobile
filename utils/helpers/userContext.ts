...
export async function getCurrentUserId(
  ...
  try {
    ...
    if (user?.id) {
      logger.info(LogCategory.USER, 'User ID retrieved from session', {
        userId: user.id,
      });
    ...
    if (fallbackToDefault) {
      logger.info(
        LogCategory.USER,
        'No user found in session, falling back to default user',
      );
      ...
      if (result.success && result.user) {
        ...
        logger.info(LogCategory.USER, 'Default user retrieved from database', {
          userId,
        });
