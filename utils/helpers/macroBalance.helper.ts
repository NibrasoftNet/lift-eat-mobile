 ...
 */
export function calculateMacroPercentages(macros: MacroNutrientsBase): MacroPercentages {
  // Calculer les calories pour chaque macronutriment
  const proteinCalories = macros.protein * CalorieFactors.PROTEIN_CALORIES_PER_GRAM;
  ...
  return {
    ...
    totalPercentage: Math.round(proteinPercentage + carbsPercentage + fatPercentage)
  };
}
...
export function evaluateMacroBalance(macros: MacroNutrientsBase): MacroBalance {
  try {
    ...
    const isOverallBalanced = isProteinOptimal && isCarbsOptimal && isFatOptimal;
    ...
    if (!isProteinOptimal) {
      if (percentages.proteinPercentage < MacroBalanceThresholds.PROTEIN.MIN) {
        recommendations.push(`Augmenter les protéines: ${percentages.proteinPercentage}% → ${MacroBalanceThresholds.PROTEIN.MIN}-${MacroBalanceThresholds.PROTEIN.MAX}%`);
      } else {
        recommendations.push(`Réduire les protéines: ${percentages.proteinPercentage}% → ${MacroBalanceThresholds.PROTEIN.MIN}-${MacroBalanceThresholds.PROTEIN.MAX}%`);
      }
    }
    
    if (!isCarbsOptimal) {
      if (percentages.carbsPercentage < MacroBalanceThresholds.CARBS.MIN) {
        recommendations.push(`Augmenter les glucides: ${percentages.carbsPercentage}% → ${MacroBalanceThresholds.CARBS.MIN}-${MacroBalanceThresholds.CARBS.MAX}%`);
      } else {
        recommendations.push(`Réduire les glucides: ${percentages.carbsPercentage}% → ${MacroBalanceThresholds.CARBS.MIN}-${MacroBalanceThresholds.CARBS.MAX}%`);
      }
    }
    
    if (!isFatOptimal) {
      if (percentages.fatPercentage < MacroBalanceThresholds.FAT.MIN) {
        recommendations.push(`Augmenter les lipides: ${percentages.fatPercentage}% → ${MacroBalanceThresholds.FAT.MIN}-${MacroBalanceThresholds.FAT.MAX}%`);
      } else {
        recommendations.push(`Réduire les lipides: ${percentages.fatPercentage}% → ${MacroBalanceThresholds.FAT.MIN}-${MacroBalanceThresholds.FAT.MAX}%`);
      }
    }
    ...
    return {
      percentages: { proteinPercentage: 0, carbsPercentage: 0, fatPercentage: 0, totalPercentage: 0 },
...
export function getMacroBalanceColor(
  ...
  const isMildlyUnbalanced = 
    (value < min && value >= min * 0.7) || 
    (value > max && value <= max * 1.3);
