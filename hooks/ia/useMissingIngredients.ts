...
import { missingIngredientApiService, SuggestionStatus } from '@/utils/services/api/missingIngredientApi.service';
...
export function useMissingIngredients(): MissingIngredientsResult {
   ...
   */
  const updateState = useCallback((partialState: Partial<MissingIngredientsState>) => {
  ...
  const loadSavedSuggestions = useCallback(async () => {
    try {
      ...
      if (!response.success || !response.suggestions) {
        throw new Error(response.error || 'Échec du chargement des suggestions');
      }
      
      const items: MissingIngredientItem[] = response.suggestions.map(suggestion => ({
      ...
      logger.endOperation('loadSavedSuggestions', 'loadSavedSuggestions', 'success', {
        count: items.length
      });
    } catch (error) {
      logger.error(`Erreur lors du chargement des suggestions: ${error instanceof Error ? error.message : String(error)}`,
                  'loadSavedSuggestions', error);
      ...
      logger.endOperation('loadSavedSuggestions', 'loadSavedSuggestions', 'failure');
    }
  }, [updateState]);
   ...
   */
  const addMissingIngredients = useCallback(async (names: string[]) => {
    try {
      ...
      const newIngredients = names.filter(name => !existingNames.has(name.toLowerCase()));
      
      if (newIngredients.length === 0) {
        logger.debug('Aucun nouvel ingrédient à ajouter', 'addMissingIngredients');
        logger.endOperation('addMissingIngredients', 'addMissingIngredients', 'success', { added: 0 });
        return;
      }
      ...
      for (const name of newIngredients) {
        ...
        const suggestionsResult = await missingIngredientApiService.getSuggestions();
        
        if (!suggestionsResult.success || !suggestionsResult.suggestions) {
          logger.warn(`Impossible de vérifier les suggestions existantes pour ${name}`, 'addMissingIngredients');
          continue;
        }
        ...
        if (existingSuggestion) {
          newItems.push({
            ...
            status: existingSuggestion.status as 'pending' | 'created' | 'ignored'
          });
        } else {
          ...
          if (result.success && result.suggestionId) {
            ...
            logger.error(`Échec de la création de suggestion pour ${name}`, 'addMissingIngredients');
          }
        }
      }
      ...
      logger.endOperation('addMissingIngredients', 'addMissingIngredients', 'success', { 
        added: newIngredients.length,
        total: state.items.length + newItems.length
      });
    } catch (error) {
      logger.error(`Erreur lors de l'ajout d'ingrédients manquants: ${error instanceof Error ? error.message : String(error)}`,
                  'addMissingIngredients', error);
      
      updateState({
        error: error as Error
      });

      logger.endOperation('addMissingIngredients', 'addMissingIngredients', 'failure');
    }
   ...
   */
  const createIngredient = useCallback(async (name: string, data?: any) => {
    try {
      ...
      logger.endOperation('createIngredient', 'createIngredient', 'success', { name });
    } catch (error) {
      logger.error(`Erreur lors de la création de l'ingrédient: ${error instanceof Error ? error.message : String(error)}`,
   ...
   */
  const ignoreIngredient = useCallback(async (name: string) => {
    try {
      ...
      logger.endOperation('ignoreIngredient', 'ignoreIngredient', 'success', { name });
    } catch (error) {
      logger.error(`Erreur lors de l'ignorance de l'ingrédient: ${error instanceof Error ? error.message : String(error)}`,
                  'ignoreIngredient', error);
