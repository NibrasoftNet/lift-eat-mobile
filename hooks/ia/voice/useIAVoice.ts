...
export const useIAVoice = (initialAutoRead: boolean = false) => {
  ...
  const [autoReadEnabled, setAutoReadEnabled] =
    useState<boolean>(initialAutoRead);
  ...
  useEffect(() => {
    try {
      ...
      logger.error(
        LogCategory.IA,
        `Erreur lors de la configuration de la lecture automatique: ${
  ...
  const generateResponse = useCallback(
    async (prompt: string, forceVoiceOutput: boolean = false) => {
      try {
        setIsLoading(true);
        setError(null);

        const response = await IAVoiceService.generateResponseWithVoice(
          prompt,
          forceVoiceOutput,
        );
        ...
        logger.error(
          LogCategory.IA,
          `Erreur lors de la génération de réponse vocale: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
  ...
  const analyzeNutritionHabits = useCallback(
    async (forceVoiceOutput: boolean = false) => {
      try {
        setIsLoading(true);
        setError(null);

        const response = await IAVoiceService.analyzeNutritionHabitsWithVoice(
          forceVoiceOutput,
        );
        ...
        setIsLoading(false);
        setError(
          `Erreur lors de l'analyse des habitudes: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        logger.error(
          LogCategory.IA,
          `Erreur lors de l'analyse vocale des habitudes: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
  ...
  const generateNutritionPlan = useCallback(
    async (
      goal: string,
      preferences?: {
        mealCount?: number;
      ...
      try {
        ...
        logger.error(
          LogCategory.IA,
          `Erreur lors de la génération vocale du plan: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
  ...
  const speakText = useCallback(
    async (
      ...
      try {
        ...
        setIsSpeaking(false);
        setError(
          `Erreur lors de la lecture vocale: ${
            err instanceof Error ? err.message : String(err)
          }`,
        );
        logger.error(
  ...
  const stopSpeaking = useCallback(async () => {
    try {
      ...
      setError(
        `Erreur lors de l'arrêt de la synthèse: ${
          err instanceof Error ? err.message : String(err)
        }`,
      );
      logger.error(
