 ...
 */
export type MealGenerationResult = [MealGenerationState, MealGenerationActions];
...
export function useMealGeneration(): MealGenerationResult {
  ...
  const updateState = useCallback(
    (partialState: Partial<MealGenerationState>) => {
  ...
  const buildMealPrompt = (criteria: MealGenerationCriteria): string => {
    ...
    if (
      ...
      criteria.includedIngredients.length > 0
    ) {
  ...
  const generateMeal = useCallback(
    async (newCriteria: MealGenerationCriteria) => {
      try {
