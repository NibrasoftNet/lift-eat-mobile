...
export interface PlanGenerationCriteria {
  ...
  macroRatios?: {                          // Ratios de macronutriments souhaités
...
export function usePlanGeneration(): PlanGenerationResult {
   ...
   */
  const updateState = useCallback((partialState: Partial<PlanGenerationState>) => {
   ...
   */
  const generatePlan = useCallback(async (newCriteria: PlanGenerationCriteria) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        updateState({
          loading: false,
          error: new Error(result.error || 'Échec de génération du plan nutritionnel'),
          retryCount: state.retryCount + 1
        });
      }
      ...
      logger.error(`Erreur lors de la génération du plan: ${error instanceof Error ? error.message : String(error)}`, 
                   'generatePlan', error);
