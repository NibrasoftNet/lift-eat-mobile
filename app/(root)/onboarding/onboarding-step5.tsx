...
export default function OnboardingStep5() {
  ...
  const handleComplete = async () => {
    try {
      console.log('[DEBUG - STEP5] Tentative de sauvegarde des données finales d\'onboarding');
      console.log('[DEBUG - STEP5] Utilisateur actuel:', currentUser, 'ID:', currentUser?.id);
      ...
      if (!userId) {
        console.warn('[DEBUG - STEP5] ATTENTION: ID utilisateur manquant dans le contexte, tentative de récupération depuis AsyncStorage...');
        ...
        try {
          ...
          if (storedUserId) {
            ...
            console.log('[DEBUG - STEP5] ID utilisateur récupéré depuis AsyncStorage:', userId);
          } else {
            console.error('[DEBUG - STEP5] Aucun ID utilisateur trouvé dans AsyncStorage');
          }
        } catch (asyncError) {
          console.error('[DEBUG - STEP5] Erreur lors de la récupération de l\'ID depuis AsyncStorage:', asyncError);
        }
      }
      ...
      console.log('[DEBUG - STEP5] Sauvegarde des données du poids cible:', { targetWeight, unit });
      ...
      if (!saveResult.success) {
        console.error('[DEBUG - STEP5] Erreur lors de la sauvegarde des données:', saveResult.error);
        throw new Error(saveResult.error || 'Erreur de sauvegarde');
      }
      
      console.log('[DEBUG - STEP5] Données de poids cible sauvegardées, marquage de l\'onboarding comme terminé');
      console.log('[DEBUG - STEP5] Synchronisation avec ID utilisateur:', userId);
      ...
      if (!completeResult.success) {
        console.error('[DEBUG - STEP5] Erreur lors de la complétion de l\'onboarding:', completeResult.error);
        throw new Error(completeResult.error || 'Erreur de complétion');
      }
      
      console.log('[DEBUG - STEP5] Onboarding terminé avec succès, redirection vers /analytics');
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.color('background') }]}>
      ...
      <View style={styles.progressHeader}>
        <Pressable onPress={handleBack} style={styles.backButton}>
          <Text style={[styles.backButtonText, { color: theme.color('primary') }]}>←</Text>
        </Pressable>
        ...
        <Text style={[styles.progressText, { color: theme.color('primary') }]}>5 / 5</Text>
      </View>
      
      {/* Titre principal */}
      <Text style={[styles.title, { color: theme.color('primary') }]}>What's your target weight?</Text>
