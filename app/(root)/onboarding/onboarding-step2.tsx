...
import { View, Text, StyleSheet, SafeAreaView, TouchableOpacity, Pressable, Dimensions, FlatList, Animated } from 'react-native';
...
export default function OnboardingStep2() {
  ...
  const months = Array.from({ length: 31 }, (_, i) => (i + 1).toString().padStart(2, '0')); // Pour démarrer à 1
    ...
    const loadSavedData = async () => {
      try {
        ...
        if (result.success && result.data && result.data.birthDate) {
          console.log('[DEBUG - STEP2] Données trouvées:', result.data.birthDate);
        ...
        console.error('[DEBUG - STEP2] Erreur lors du chargement des données d\'onboarding:', error);
      }
    };
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      console.log('[DEBUG - STEP2] Date de naissance sélectionnée:', birthDate.toISOString());
      ...
      if (!result.success) {
        console.error('[DEBUG - STEP2] Erreur lors de la sauvegarde:', result.error);
        throw new Error(result.error || 'Erreur de sauvegarde');
      }
      
      console.log('[DEBUG - STEP2] Date de naissance sauvegardée, passage à l\'étape 3');
  ...
  const renderDateItem = (type: DateSelectionType, selectedIndex: number) => ({ item, index }: { item: string, index: number }) => {
    ...
    return (
      ...
      >
        <Text 
          style={[
            styles.dateText, 
            isSelected && [styles.selectedDateText, { color: theme.color('successLighter') }]
          ]}
        >
          {item}
        </Text>
  ...
  const renderDateColumn = (type: DateSelectionType, options: string[], selectedIndex: number, label: string, listRef: React.RefObject<FlatList>) => {
    return (
      <View style={styles.dateColumn}>
        <Text style={[styles.columnLabel, { color: theme.color('primary') }]}>{label}</Text>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.color('background') }]}>
      ...
      <View style={styles.progressHeader}>
        ...
        <Text style={[styles.progressText, { color: theme.color('primary') }]}>2 / 5</Text>
      </View>
      
      {/* Titre principal */}
      <Text style={[styles.title, { color: theme.color('primary') }]}>When's your birthday?</Text>
