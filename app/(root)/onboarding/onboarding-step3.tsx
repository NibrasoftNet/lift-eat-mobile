...
export default function OnboardingStep3() {
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        console.error('[DEBUG - STEP3] Erreur lors du chargement des données d\'onboarding:', error);
      }
    };
  ...
  const handleContinue = async () => {
    try {
      ...
      if (!result.success) {
        console.error('[DEBUG - STEP3] Erreur lors de la sauvegarde:', result.error);
      ...
      console.error('[DEBUG - STEP3] Erreur lors de la sauvegarde des données d\'onboarding:', error);
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.color('background') }]}>
      ...
      <View style={styles.progressHeader}>
        ...
        <Text style={[styles.progressText, { color: theme.color('primary') }]}>3 / 5</Text>
      </View>
      
      {/* Titre principal */}
      <Text style={[styles.title, { color: theme.color('primary') }]}>How tall are you?</Text>
