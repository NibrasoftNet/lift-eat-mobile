...
export default function OnboardingStep4() {
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        console.error('[DEBUG - STEP4] Erreur lors du chargement des données d\'onboarding:', error);
      }
    };
  ...
  const handleContinue = async () => {
    try {
      console.log('[DEBUG - STEP4] Sauvegarde du poids actuel:', { weight, unit });
      ...
      if (!result.success) {
        console.error('[DEBUG - STEP4] Erreur lors de la sauvegarde:', result.error);
        throw new Error(result.error || 'Erreur de sauvegarde');
      }
      
      console.log('[DEBUG - STEP4] Poids actuel sauvegardé, passage à l\'étape 5');
      
      // Naviguer vers l'écran suivant
      router.push('/onboarding/onboarding-step5');
    } catch (error) {
      console.error('[DEBUG - STEP4] Erreur lors de la sauvegarde des données d\'onboarding:', error);
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.color('background') }]}>
      ...
      <View style={styles.progressHeader}>
        ...
        <Text style={[styles.progressText, { color: theme.color('primary') }]}>4 / 5</Text>
      </View>
      
      {/* Titre principal */}
      <Text style={[styles.title, { color: theme.color('primary') }]}>What's your current weight?</Text>
