...
export const NutritionAdjuster: React.FC<NutritionAdjusterProps> = ({
  ...
  const [normalizedNutrition, setNormalizedNutrition] =
  ...
  useEffect(() => {
    if (useMcpMode && mealId) {
      setMcpLoading(true);
      nutritionEngine
        .getMealNutrition(mealId, 100, NutritionDisplayMode.PER_100G)
        .then(
          (result: {
            success: boolean;
            error?: string;
  ...
  const isBalanced = () => {
    ...
    const caloriesDiff =
      Math.abs(calculatedCalories - macros.calories) / macros.calories;
    ...
    try {
      ...
      if (nutrient === 'calories') {
        ...
        const calculatedMacros = {
          calories: sanitizedValue,
          protein:
            (sanitizedValue * (ratio.protein / 100)) /
            MacroCalorieFactors.PROTEIN,
          carbs:
            (sanitizedValue * (ratio.carbs / 100)) / MacroCalorieFactors.CARBS,
        ...
        if (newTotal > 0) {
          ...
          const total =
            updatedRatio.protein + updatedRatio.carbs + updatedRatio.fat;
          if (total !== 100) {
  ...
  const applyRatioPreset = (preset: (typeof ratioPresets)[0]) => {
    ...
    const newMacros = {
      ...
      protein:
        (macros.calories * (preset.protein / 100)) /
        MacroCalorieFactors.PROTEIN,
      carbs:
        (macros.calories * (preset.carbs / 100)) / MacroCalorieFactors.CARBS,
  ...
  const updateRatio = (
    ...
    const others = Object.keys(current).filter((k) => k !== nutrient) as Array<
      'protein' | 'carbs' | 'fat'
    >;
    ...
    if (userUpdates) {
      ...
      const newMacros = {
        calories: macros.calories,
        protein:
          (macros.calories * (newRatio.protein / 100)) /
          MacroCalorieFactors.PROTEIN,
        carbs:
          (macros.calories * (newRatio.carbs / 100)) /
          MacroCalorieFactors.CARBS,
    ...
    <Card className={`p-4 ${className}`}>
      ...
      <VStack className="mb-4">
        <HStack className="justify-between items-center mb-1">
          ...
          <AnimatedText style={caloriesStyle}>
      ...
      <VStack>
        ...
        <HStack className="gap-2">
          <VStack className="flex-1">
            <Text className="text-xs text-center text-blue-600 mb-1">
              Protéines
            </Text>
          ...
          <VStack className="flex-1">
            <Text className="text-xs text-center text-amber-600 mb-1">
              Glucides
            </Text>
            <Box className="bg-gray-100 rounded-md px-2 py-1">
              <TextInput
          ...
          <VStack className="flex-1">
            <Text className="text-xs text-center text-green-600 mb-1">
              Lipides
            </Text>
            <Box className="bg-gray-100 rounded-md px-2 py-1">
              <TextInput
