...
const MealGeneratorForm: React.FC<MealGeneratorFormProps> = ({
  ...
  const [formMethods, formActions] = useMealGeneratorForm(
    uiActions,
    onMealGenerated,
  );
  ...
  const [generatedMeal, setGeneratedMeal] = React.useState<IaMealType | null>(
    null,
  );
  ...
  const onSubmit = async (data: any) => {
    try {
      logger.info('Début de la génération du repas', 'onSubmit', {
      ...
      if (meal) {
        ...
        if (onMealGenerated && typeof onMealGenerated === 'function') {
          ...
          logger.warn(
            'Callback onMealGenerated non défini ou non valide',
            'onSubmit',
          );
        }
      } else {
      ...
      <VStack space="md">
        ...
        <Pressable
          style={{
            ...
            marginTop: 16,
            opacity:
              uiState.loading || Object.keys(errors).length > 0 ? 0.7 : 1,
          }}
