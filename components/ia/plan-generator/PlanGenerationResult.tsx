...
const PlanGenerationResult: React.FC<PlanGenerationResultProps> = ({
  plan,
  loading,
  error,
}) => {
  const [expandedDay, setExpandedDay] = React.useState<number | null>(null);
  const [expandedMeal, setExpandedMeal] = React.useState<{
    day: number;
    meal: string;
  } | null>(null);
  ...
  const errorBackgroundColor = useThemeColor(
    { light: '#ffebee', dark: '#3e2c2c' },
    'background',
  );
  const errorTextColor = useThemeColor(
    { light: '#d32f2f', dark: '#ff6b6b' },
    'text',
  );
  ...
  const toggleMealExpansion = (dayIndex: number, mealName: string) => {
    const isExpanded =
      expandedMeal?.day === dayIndex && expandedMeal?.meal === mealName;
  ...
  const renderMeal = (meal: IaMealType, dayIndex: number) => {
    const isExpanded =
      expandedMeal?.day === dayIndex && expandedMeal?.meal === meal.type;
