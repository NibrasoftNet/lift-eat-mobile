...
export const SimpleMacroChart: React.FC<SimpleMacroChartProps> = ({
  ...
  return (
    <Box className={`p-4 ${className}`}>
      <Text className="text-lg font-semibold mb-3">Répartition des macronutriments</Text>
      ...
      <VStack className="gap-3">
        ...
        <VStack className="gap-1">
          <HStack className="justify-between">
            ...
            <Text className="text-sm font-semibold">{validProtein}g ({Math.round(proteinPercentage)}%)</Text>
          </HStack>
          <HStack className="h-4 bg-gray-200 rounded-full overflow-hidden">
            <Box 
              className={`h-full ${isProteinOptimal ? 'bg-blue-500' : 'bg-blue-300'}`} 
              style={{ width: `${Math.min(100, proteinPercentage)}%` }} 
            />
          </HStack>
          <Text className={`text-xs ${isProteinOptimal ? 'text-green-600' : 'text-amber-500'}`}>
        ...
        <VStack className="gap-1">
          <HStack className="justify-between">
            ...
            <Text className="text-sm font-semibold">{validCarbs}g ({Math.round(carbsPercentage)}%)</Text>
          </HStack>
          <HStack className="h-4 bg-gray-200 rounded-full overflow-hidden">
            <Box 
              className={`h-full ${isCarbsOptimal ? 'bg-amber-500' : 'bg-amber-300'}`} 
              style={{ width: `${Math.min(100, carbsPercentage)}%` }} 
            />
          </HStack>
          <Text className={`text-xs ${isCarbsOptimal ? 'text-green-600' : 'text-amber-500'}`}>
        ...
        <VStack className="gap-1">
          <HStack className="justify-between">
            ...
            <Text className="text-sm font-semibold">{validFat}g ({Math.round(fatPercentage)}%)</Text>
          </HStack>
          <HStack className="h-4 bg-gray-200 rounded-full overflow-hidden">
            <Box 
              className={`h-full ${isFatsOptimal ? 'bg-green-500' : 'bg-green-300'}`} 
              style={{ width: `${Math.min(100, fatPercentage)}%` }} 
            />
          </HStack>
          <Text className={`text-xs ${isFatsOptimal ? 'text-green-600' : 'text-amber-500'}`}>
      ...
      <VStack className="mt-4 p-2 bg-gray-50 rounded-md">
        ...
        <Text className="text-xs">Calories estimées: {(validProtein * 4) + (validCarbs * 4) + (validFat * 9)} kcal</Text>
      </VStack>
    </Box>
  );
};
