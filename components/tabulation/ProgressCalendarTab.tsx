...
const ProgressCalendarTab = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      logger.info(
        ...
        'Fetching user data for progress calendar',
      );
      ...
      try {
        ...
        if (userId) {
          ...
          if (result.success && result.data && result.data.user) {
            ...
            logger.warn(
              LogCategory.DATABASE,
              `Failed to get user with ID ${userId}, trying default user`,
            );
          }
        }
        ...
        logger.error(
          LogCategory.DATABASE,
          'No user ID found or retrieval failed',
        );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        logger.info(
          LogCategory.DATABASE,
          `Fetching current plan for user ${userId} via MCP`,
        );
        ...
        if (!result.success) {
          logger.error(
            LogCategory.DATABASE,
            `Failed to get current plan: ${result.error}`,
          );
          return null;
        }
  ...
  const { data: progressDays, isLoading: isProgressLoading } = useQuery<
    DailyProgressOrmProps[]
  >({
    ...
    queryFn: async () => {
      ...
      logger.info(
        LogCategory.DATABASE,
        'Fetching progress days for current plan via service',
        {
          planId: currentPlan.id,
        },
      );

      const result = await progressPagesService.getProgressByPlan(
        currentPlan.id,
      );
      ...
      if (!result.success || !result.data) {
        logger.warn(
          LogCategory.DATABASE,
          'No progress days found for current plan',
          {
            planId: currentPlan.id,
          },
        );
  ...
  const { data: planResult, isLoading: isPlanDetailsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      logger.info(
        LogCategory.DATABASE,
        'Fetching plan details to display calendar markers',
        {
          userId,
          planId: currentPlan.id,
        },
      ...
      const dailyPlansData =
        result.success && result.data ? result.data.dailyPlans : [];
      ...
      logger.debug(LogCategory.DATABASE, 'Plan details fetched', {
        ...
        firstPlanDetails: dailyPlansData?.[0]
          ? Object.keys(dailyPlansData[0])
          : [],
      });
  ...
  const markedDates = React.useMemo(() => {
    ...
    logger.debug(LogCategory.UI, 'Calendar plan data debug', {
      ...
      dailyPlansType: planResult?.dailyPlans
        ? typeof planResult.dailyPlans
        : 'undefined',
      dailyPlansLength: planResult?.dailyPlans?.length,
      firstPlanDetails: planResult?.dailyPlans?.[0]
        ? Object.keys(planResult.dailyPlans[0])
        : [],
    });
    ...
    if (
      planResult?.dailyPlans &&
      Array.isArray(planResult.dailyPlans) &&
      planResult.dailyPlans.length > 0
    ) {
      logger.info(
        LogCategory.UI,
        `Processing ${planResult.dailyPlans.length} daily plans for calendar markers`,
      );

      // Parcourir tous les plans journaliers
      planResult.dailyPlans.forEach(
        (dailyPlan: DailyPlanOrmProps, index: number) => {
          ...
          logger.debug(
            LogCategory.UI,
            `Converted ${dailyPlan.day} to date: ${planDate}`,
          );
      ...
      logger.info(
        LogCategory.UI,
        `Added markers for ${Object.keys(dates).length} days`,
      );
    }
  ...
  const finalMarkedDates = React.useMemo(() => {
    ...
    if (selectedDate && !markedDatesWithSelection[selectedDate]) {
      ...
      logger.debug(
        LogCategory.UI,
        `Added selection marker for date: ${selectedDate}`,
      );
    }
  ...
  const handleDateSelect = async (day: any) => {
    try {
      ...
      logger.info(
        LogCategory.DATABASE,
        'Récupération de la progression des repas via progress-pages service',
        {
          userId: userId,
          date: day.dateString,
        },
      ...
      logger.info(
        LogCategory.UI,
        `Date sélectionnée ${day.dateString} correspond au jour: ${selectedDayOfWeek}`,
      );
      ...
      if (currentPlan && currentPlan.id) {
        ...
        const dailyPlanDetails = await planPagesService.getPlanDetails(
          currentPlan.id,
        );

        // Extraire les données du plan pour faciliter l'accès et corriger les erreurs TypeScript
        const dailyPlanData =
          dailyPlanDetails.success && dailyPlanDetails.data
            ? dailyPlanDetails.data.dailyPlans
            : [];

        logger.info(
          LogCategory.UI,
          `Détails du plan récupérés pour le plan ${currentPlan.id}`,
          {
            planSuccess: dailyPlanDetails.success,
            dailyPlansCount: dailyPlanData?.length || 0,
            dailyPlanIds:
              dailyPlanData?.map((dp: { id: number }) => dp.id) || [],
          },
        );

        if (
          dailyPlanDetails.success &&
          dailyPlanData &&
          ...
          logger.info(
            LogCategory.UI,
            `Plans journaliers trouvés pour ${selectedDayOfWeek}`,
            {
          ...
          const dailyPlanWithMeals = matchingDailyPlans.find(
            (dp: { meals?: any[] }) => dp.meals && dp.meals.length > 0,
          );

          if (dailyPlanWithMeals) {
            logger.info(
              LogCategory.UI,
              `Trouvé un plan journalier avec repas: ${dailyPlanWithMeals.id}`,
              {
                mealsCount: dailyPlanWithMeals.meals.length,
                mealsData: JSON.stringify(
                  dailyPlanWithMeals.meals.map((m) => ({
                    id: m.id,
                    name: m.name,
                  })),
                ),
            ...
            if (!dailyProgress && currentPlan && selectedDate) {
              ...
              try {
                logger.info(
                  LogCategory.DATABASE,
                  `Création d'une progression quotidienne pour la date ${selectedDate}`,
                );
                // Utiliser la méthode createDailyProgress du service progressPagesService
                const createResult =
                  await progressPagesService.createDailyProgress(selectedDate);

                if (createResult.success && createResult.data) {
                  logger.info(
                    LogCategory.DATABASE,
                    `Progression quotidienne créée avec succès: ${createResult.data.progress.id}`,
                  );
                  ...
                  logger.error(
                    LogCategory.DATABASE,
                    `Échec de création de la progression: ${createResult.error}`,
                  );
                }
              } catch (error) {
                logger.error(
            ...
            const mealsWithProgressFormat = dailyPlanWithMeals.meals.map(
              (meal) => {
            ...
            logger.warn(
              LogCategory.UI,
              `Aucun plan journalier pour ${selectedDayOfWeek} ne contient de repas`,
            );
          }
        } else {
          ...
          logger.warn(
            ...
            `Erreur lors de la récupération des détails du plan: ${
              dailyPlanDetails.error || 'unknown error'
            }`,
          );
        }
      } else {
        ...
        logger.warn(
          LogCategory.UI,
          `Plan courant non trouvé ou sans ID valide`,
        );
      }

      // Récupérer aussi les progrès pour cette date
      const result = await progressPagesService.getDailyProgress(
        day.dateString,
      );
      ...
      <VStack>
        ...
        <Center className="flex-1 p-4">
          ...
          <Text className="text-center mb-4">
            Veuillez définir un plan comme courant dans l'onglet Plans pour
            ...
            <Animated.View entering={FadeInRight.duration(300)}>
              <Box className="px-4 py-4 mt-2">
                  ...
                  <HStack space="md" className="mb-2">
                    <Box className="mr-2">
                      <Text className="text-sm text-gray-600">
                        Progression:{' '}
                        {dailyProgress.pourcentageCompletion.toFixed(0)}%
                      </Text>
                    </Box>
                    <Box>
                      <Text className="text-sm text-gray-600">
                ...
                {mealsWithProgress.length === 0 ? (
                  <Text className="mt-2">
                    Aucun repas programmé pour cette date.
                  </Text>
                ) : (
                  <Box className="mt-3" style={{ minHeight: 300 }}>
                    <Text className="text-blue-800 mb-2">
                      Nombre de repas trouvés: {mealsWithProgress.length}
                    </Text>
                    <MealsClickSelection
