...
const SimpleMealCard: React.FC<{
  ...
  return (
    <Animated.View entering={FadeInUp.delay(index * 50)} className="mb-2">
      <Pressable
        onPress={onToggle}
...
const MealsBottomSheet: React.FC<MealsDrawerProps> = ({
  showMealsDrawer,
  setShowMealsDrawer,
  dailyPlanId,
  planId,
  ...
  const toggleMealSelection = (meal: MealOrmProps) => {
    if (selectedMeals.some((selected) => selected.id === meal.id)) {
      ...
      setSelectedMeals(
        selectedMeals.filter((selected) => selected.id !== meal.id),
      );
    } else {
  ...
  const handleAddMealsToPlan = async () => {
    if (selectedMeals.length === 0) {
      Toast.show({
        type: 'infos',
        text1: 'Meal',
        text2: `Veuillez sélectionner au moins un repas`,
      });
      return;
    }
  ...
  return (
    <BottomSheet
      open={showMealsDrawer}
      setOpen={setShowMealsDrawer}
      height="sm"
    ...
    >
      <VStack>
        ...
        <VStack className="gap-2 flex-1">
          {/* Champ de recherche */}
          <Input className="bg-white/90 rounded-xl h-12 p-1">
              ...
              <>
                ...
                {cuisineOptions.map((cuisineType) => (
                  <VStack
                    ...
                    className="w-14 h-16 items-center"
                  >
                    <Button
              ...
              {mealsList?.length === 0 ? (
                <VStack className="items-center justify-center p-4 h-40">
                  ...
                  <Text className="text-gray-500 mt-2">
                    Aucun repas trouvé. Ajustez vos filtres.
                  </Text>
                </VStack>
              ) : (
                <FlashList
...
export default MealsBottomSheet;
