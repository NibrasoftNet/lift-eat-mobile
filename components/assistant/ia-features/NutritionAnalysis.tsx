...
const NutritionAnalysis: React.FC<NutritionAnalysisProps> = ({ onAnalysisComplete }) => {
        ...
        toast.show({
          render: ({ id }) => (
            <MultiPurposeToast
              ...
              description={result.error || "Nous avons pu générer une analyse de base, mais certains détails peuvent manquer."}
            />
          ),
        });
      }
  ...
  const renderAnalysisContent = () => {
    if (analyzing) {
      return (
        <Box className="items-center justify-center p-6">
          ...
          <Text className="text-center text-base mt-4 mb-6">Analyse de vos habitudes nutritionnelles en cours...</Text>
          <Box className="w-full h-2 bg-gray-200 rounded overflow-hidden mb-4">
            <Box className="w-3/5 h-full bg-primary-500" />
          </Box>
          <Text className="text-xs opacity-60">Cette opération peut prendre quelques instants</Text>
        </Box>
      );
    }
    ...
    if (nutritionAnalysis.text) {
      return (
        <Box className="p-4">
          <Text className="text-base leading-relaxed mb-6">{nutritionAnalysis.text}</Text>
          ...
          {nutritionAnalysis.improvements.length > 0 && (
            <Box className="mb-6">
              <Text className="text-lg font-bold mb-3">Axes d'amélioration</Text>
              {nutritionAnalysis.improvements.map((improvement, index) => (
                <HStack key={index} className="items-center mb-3">
                  ...
                  <Text className="flex-1 text-base leading-5">{improvement}</Text>
                </HStack>
              ))}
            </Box>
          )}
            ...
            <Box className="mb-6">
              ...
              {nutritionAnalysis.recommendations.map((recommendation, index) => (
                <HStack key={index} className="items-center mb-3">
                  ...
                  <Text className="flex-1 text-base leading-5">{recommendation}</Text>
                </HStack>
              ))}
            </Box>
          )}
    ...
    return (
      <Box className="items-center justify-center p-6">
        <Text className="text-lg font-bold mb-4 text-center">Analyse Nutritionnelle</Text>
        ...
        <Button onPress={analyzeNutrition} className="bg-primary-500 rounded-lg">
