...
const IngredientsDrawer = React.memo(
  ({
    // Utiliser React.memo explicitement pour éviter les re-rendus inutiles
    ...
    useEffect(() => {
      ...
      ingredientDrawerUIService.debounceSearchTerm(
        searchTerm,
        updateDebouncedTerm,
        DEBOUNCE_DELAY,
      );
    ...
    const fetchIngredients = useCallback(
      async ({ pageParam = 1 }) => {
        try {
          ...
          const result = await ingredientDrawerUIService.fetchIngredients(
            params,
          );
          ...
          logger.error(
            LogCategory.APP,
            `IngredientsDrawer - Error fetching ingredients: ${error}`,
          );
    ...
    const renderItem = useCallback(
      ({ item, index }: { item: IngredientWithUniqueId; index: number }) => {
