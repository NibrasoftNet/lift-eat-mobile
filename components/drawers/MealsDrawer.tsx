          ...
          <VStack space="xs">
            ...
            <HStack space="sm">
              {isSelected && onMealTypeChange ? (
                ...
                >
                  ...
                  <SelectPortal>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem
                        label={mealDrawerUIService.getMealTypeName(MealTypeEnum.BREAKFAST)}
                        value={MealTypeEnum.BREAKFAST}
                      />
                      <SelectItem
                        label={mealDrawerUIService.getMealTypeName(MealTypeEnum.LUNCH)}
                        value={MealTypeEnum.LUNCH}
                      />
                      <SelectItem
                        label={mealDrawerUIService.getMealTypeName(MealTypeEnum.DINNER)}
                        value={MealTypeEnum.DINNER}
                      />
                      <SelectItem
                        label={mealDrawerUIService.getMealTypeName(MealTypeEnum.SNACK)}
                ...
                <View
                  className={`rounded-full px-2 py-1 ${mealDrawerUIService.getMealTypeColor(mealType || item.type).bgColor}`}
                >
                  <Text
                    className={`text-xs font-light ${mealDrawerUIService.getMealTypeColor(mealType || item.type).textColor}`}
                  >
                    {mealDrawerUIService.getMealTypeName(mealType || item.type)}
                  </Text>
                </View>
              )}
...
function MealsDrawer({
  ...
  const handleAddMealsToPlan = useCallback(async () => {
    try {
      ...
      const result = await mealDrawerUIService.addMealsToPlan(dailyPlanId, mealsToAdd);
