              ...
              <HStack space="sm">
                {isSelected && onMealTypeChange ? (
                  <Select
                    ...
                    onValueChange={(value) =>
                      onMealTypeChange(value as MealTypeEnum)
                    }
                    className="w-32"
                  >
                    <SelectTrigger>
                    ...
                    <SelectPortal>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem
                          label={mealDrawerUIService.getMealTypeName(
                  ...
                  <View
                    className={`rounded-full px-2 py-1 ${
                      mealDrawerUIService.getMealTypeColor(
                        mealType || item.type,
                      ).bgColor
                    }`}
                  >
                    ...
                    >
                      {mealDrawerUIService.getMealTypeName(
                        mealType || item.type,
                      )}
                    </Text>
                  </View>
...
function MealsDrawer({
  ...
  const queryFn = useCallback(async () => {
    ...
    if (!userId) {
      logger.error(
        LogCategory.AUTH,
        'MealsDrawer - Cannot fetch meals: User not authenticated',
      );
    ...
    try {
      ...
      if (!result.success) {
        logger.error(
          ...
          `MealsDrawer - Failed to fetch meals: ${result.error}`,
        );
      ...
      if (Array.isArray(result.meals)) {
        logger.debug(
          LogCategory.APP,
          `MealsDrawer - Received ${result.meals.length} meals`,
        );

        // Ajouter un identifiant stable et unique à chaque repas
        const mealsWithStableIds = result.meals
          .map((meal) => {
            // Vérifier si un repas est undefined et logger la source
            if (!meal) {
              logger.error(
                LogCategory.APP,
                `Meal is undefined in MealsDrawer.queryFn. Stack: ${
                  new Error().stack
                }`,
              );
              return null;
        ...
        logger.error(
          LogCategory.APP,
          'MealsDrawer - Meals is not an array in queryFn',
          result,
        );
        return [];
      }
    ...
    (meal: MealOrmProps) => {
      try {
        ...
        if (!meal) {
          logger.error(
            LogCategory.APP,
            `Meal is undefined in MealsDrawer.handleMealToggle. Stack: ${
              new Error().stack
            }`,
          );
          return;
        ...
        if (typeof meal.id !== 'number') {
          logger.error(
            LogCategory.APP,
            'MealsDrawer - Invalid meal in handleMealToggle',
            { meal },
          );
          toast.show({
            render: ({ id }) => (
        ...
        logger.error(
          LogCategory.APP,
          `MealsDrawer - Error in handleMealToggle: ${error}`,
        );
      }
    },
    [toast],
  ...
  const handleMealTypeChange = useCallback(
    (mealId: number, type: MealTypeEnum) => {
      setMealTypes((prev) => ({
        ...prev,
        [mealId]: type,
      }));
  ...
  const handleAddMealsToPlan = useCallback(async () => {
    try {
      ...
      logger.debug(
        LogCategory.APP,
        `MealsDrawer - Adding ${selectedMeals.length} meals to plan ${dailyPlanId}`,
      );
      ...
      const result = await mealDrawerUIService.addMealsToPlan(
        dailyPlanId,
        mealsToAdd,
      );
      ...
      logger.error(
        LogCategory.APP,
        `Error adding meals to plan: ${errorMessage}`,
      );
