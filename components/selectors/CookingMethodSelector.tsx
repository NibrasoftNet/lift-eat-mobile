      ...
      <VStack className="gap-4">
            ...
            return (
              <Pressable
                ...
                className={`px-3 py-2 rounded-lg ${
                  selectedMethod === method ? 'bg-primary-500' : 'bg-gray-100'
                }`}
        ...
        <VStack className="bg-gray-50 p-3 rounded-lg">
          <Text className="font-semibold">
            {methodInfo?.label || 'Méthode de cuisson'}
          </Text>
          <Text className="text-sm text-gray-600 mt-1 mb-3">
            {methodInfo?.description || 'Description non disponible'}
          </Text>
          <Text className="text-xs text-gray-500 italic">
            {methodInfo?.impact || 'Impact nutritionnel inconnu'}
          </Text>
        </VStack>
        ...
        <VStack className="mt-2">
          ...
          <HStack className="justify-between">
            <VStack className="flex-1">
              ...
              <HStack className="items-center">
                <Text className="text-sm">
                  {roundToDecimals(adjustedMacros.calories, 1)} kcal
                </Text>
                <Text
                  className={`text-xs ml-2 ${getDifferenceClass(
                    adjustmentPercentages.calories,
                  )}`}
                >
            ...
            <VStack className="flex-1">
              ...
              <HStack className="items-center">
                <Text className="text-sm">
                  {roundToDecimals(adjustedMacros.carbs, 1)} g
                </Text>
                <Text
                  className={`text-xs ml-2 ${getDifferenceClass(
                    adjustmentPercentages.carbs,
                  )}`}
                >
          ...
          <HStack className="justify-between mt-2">
            <VStack className="flex-1">
              ...
              <HStack className="items-center">
                <Text className="text-sm">
                  {roundToDecimals(adjustedMacros.protein, 1)} g
                </Text>
                <Text
                  className={`text-xs ml-2 ${getDifferenceClass(
                    adjustmentPercentages.protein,
                  )}`}
                >
            ...
            <VStack className="flex-1">
              ...
              <HStack className="items-center">
                <Text className="text-sm">
                  {roundToDecimals(adjustedMacros.fat, 1)} g
                </Text>
                <Text
                  className={`text-xs ml-2 ${getDifferenceClass(
                    adjustmentPercentages.fat,
                  )}`}
                >
          ...
          <HStack className="justify-between mt-2">
            <VStack className="flex-1">
              ...
              <HStack className="items-center">
                <Text className="text-sm">
                  {roundToDecimals(adjustedWeight, 1)} g
                </Text>
                <Text
                  className={`text-xs ml-2 ${getDifferenceClass(
                    adjustmentPercentages.weight,
                  )}`}
                >
