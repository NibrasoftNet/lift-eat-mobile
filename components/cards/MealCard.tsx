                ...
                <HStack className="items-center justify-between mb-2">
                  ...
                  </HStack>
                  {
                    (() => {
                      ...
                      const weightInGrams = item.quantity ? parseInt(item.quantity.toString()) : 200; // Valeur directe en grammes
                      
                      // Logger les valeurs pour débogage
                      logger.info(LogCategory.NUTRITION, `[MEALCARD-CALORIES] Poids: ${weightInGrams}g`);
                      
                      // État pour les données nutritionnelles
                      const [nutritionData, setNutritionData] = useState<{
                        macros: { calories: number; carbs: number; protein: number; fat: number; unit?: string };
                        displayText: string;
                      ...
                      React.useEffect(() => {
                        ...
                        const fetchNutritionData = async () => {
                          try {
                            const result = await nutritionPagesService.getMealNutritionForDisplay(
                              item.id,
                              undefined,
                ...
                {/* Utiliser nutritionPagesService pour standardiser les valeurs à 100g via MCP */}
                {
                  (() => {
                    ...
                    React.useEffect(() => {
                      ...
                      // Log pour débogage
                      logger.info(LogCategory.NUTRITION, `[MEALCARD-NUTRITION] Chargement des données nutritionnelles pour le repas ${item.id}`);
                        ...
                        try {
                          ...
                          if (isMounted) {
                            if (result.success && result.data) {
                              setNutritionData(result.data);
                              setError(null);
                              logger.debug(LogCategory.NUTRITION, `[MEALCARD-NUTRITION] Données reçues avec succès pour le repas ${item.id}`);
                            } else {
                              setError(result.error || 'Erreur lors du chargement des données');
                              setNutritionData(null);
                              logger.warn(LogCategory.NUTRITION, `[MEALCARD-NUTRITION] Erreur de données pour le repas ${item.id}`, { error: result.error });
                            }
                            setLoading(false);
                          }
                        } catch (err) {
                          if (isMounted) {
                            ...
                            setNutritionData(null);
                            setLoading(false);
                            logger.error(LogCategory.NUTRITION, `[MEALCARD-NUTRITION] Exception lors du chargement des données pour le repas ${item.id}`, { error: err });
                          }
                        }
                      };
