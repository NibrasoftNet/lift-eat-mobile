...
const MacrosInfoCard = ({
  ...
  const [displayText, setDisplayText] = useState<string>('Valeurs nutritionnelles');
  const [percentages, setPercentages] = useState<{protein: number, carbs: number, fat: number}>({
    protein: 0,
    carbs: 0,
    fat: 0
  });
  ...
  const { data: mealNutritionData, isLoading: mealIsLoading, isError: mealIsError } = useQuery({
    queryKey: [`macro-breakdown-${mealId || 0}-${quantity || 0}-${displayMode}`],
    queryFn: async () => {
      // S'assurer que mealId est défini
      if (!mealId) throw new Error('ID de repas non défini');
      
      logger.info(LogCategory.UI, `Récupération des macros du repas ${mealId} pour affichage`);
      ...
      if (!result.success || !result.data) {
        throw new Error(result.error || 'Erreur lors de la récupération des macros');
      }
  ...
  return (
    <Card className="gap-2">
      <HStack className="items-center justify-between">
        <VStack className="gap-1">
          ...
          <Text className="text-xs text-gray-600 font-semibold">
            {isLoading ? 'Chargement...' : isError ? 'Erreur de calcul' : displayText}
          </Text>
        </VStack>
        {/* Calories */}
        <NutritionBox
          ...
          tooltipContent={`Énergie totale: ${!isLoading && !isError ? Math.round(normalizedMacros.calories) : '0'} KCal`}
        />
      </HStack>
      ...
      <HStack className="justify-end pt-2">
        ...
        >
          <ButtonText>{detailedView ? 'Vue simple' : 'Vue détaillée'}</ButtonText>
        </Button>
      </HStack>
      ...
      {detailedView ? (
        ...
        <NutritionLabel
          ...
          title={displayText}
          quantity={displayMode === NutritionDisplayMode.PER_100G ? 100 : mealWeight}
          quantityUnit={unit}
        ...
        <HStack className="justify-around pt-3 border-t border-gray-100">
          ...
          <VStack>
            <NutritionBox
              ...
              showTooltip={true}
              tooltipContent={`${Math.round(carbsPercentage)}% des calories - Optimal: 45-65%`}
            />
            {/* Indicateur de seuil optimal */}
            <Text className={`text-xs mt-1 text-center ${getThresholdColor(isCarbsOptimal)}`}>
          ...
          <VStack>
            <NutritionBox
              ...
              showTooltip={true}
              tooltipContent={`${Math.round(fatsPercentage)}% des calories - Optimal: 20-35%`}
            />
            {/* Indicateur de seuil optimal */}
            <Text className={`text-xs mt-1 text-center ${getThresholdColor(isFatsOptimal)}`}>
          ...
          <VStack>
            <NutritionBox
              ...
              showTooltip={true}
              tooltipContent={`${Math.round(proteinPercentage)}% des calories - Optimal: 15-35%`}
            />
            {/* Indicateur de seuil optimal */}
            <Text className={`text-xs mt-1 text-center ${getThresholdColor(isProteinOptimal)}`}>
      ...
      <VStack className="mt-2 p-2 bg-gray-50 rounded-md">
        <Text className="text-sm font-semibold">Évaluation des macronutriments</Text>
        <HStack className="justify-between">
          <Text className="text-xs">Répartition calorique:</Text>
          <HStack className="gap-2">
            <Text className={`text-xs ${getThresholdColor(isProteinOptimal)}`}>P: {Math.round(proteinPercentage)}%</Text>  
            <Text className={`text-xs ${getThresholdColor(isCarbsOptimal)}`}>C: {Math.round(carbsPercentage)}%</Text>
            <Text className={`text-xs ${getThresholdColor(isFatsOptimal)}`}>L: {Math.round(fatsPercentage)}%</Text>
          </HStack>
        </HStack>
