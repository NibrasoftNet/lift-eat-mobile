...
const PlanCard: React.FC<{ item: PlanOrmProps; index: number }> = ({
  ...
  const { mutateAsync: deleteAsync, isPending: isDeletePending } = useMutation({
    mutationFn: () => planPagesService.deletePlan(item.id),
    onSuccess: async () => {
      toast.show({
        placement: 'top',
        render: ({ id }: { id: string }) => {
      ...
      // Invalider le cache des plans pour la mise à jour de l'UI
            ...
            {({ pressed }: { pressed: boolean }) => (
              ...
              >
                ...
                <MacrosDetailsBox
                  ...
                  areValuesInGrams={item.carbs > 100 || item.protein > 100}
                  unit={'g'}
                />
              </VStack>
            )}
